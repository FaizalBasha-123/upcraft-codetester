...
import { Tag } from "@agenthorsy-ai/ui/v2/badge-v2"
...
import { wslAgenthorsyAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.agenthorsyChecks[item.config.distro]
          const agenthorsyAction = () => wslAgenthorsyAction(check())
          const busy = () => wsl.data?.job?.kind === "install-agenthorsy" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={agenthorsyAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installAgenthorsy(item.config.distro))}
                    >
