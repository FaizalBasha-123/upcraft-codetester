...
Delete `packages/agenthorsy/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/agenthorsy/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/agenthorsy/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/agenthorsy`
- TUI plugin loader tests in `packages/agenthorsy` if runtime plugin API wiring changed
