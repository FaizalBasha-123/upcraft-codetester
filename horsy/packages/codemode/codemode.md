...
This is the living design and status document for `@agenthorsy-ai/codemode` and its existing V2 AgentHorsy adapter.
...
### V2 AgentHorsy adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/agenthorsy/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
