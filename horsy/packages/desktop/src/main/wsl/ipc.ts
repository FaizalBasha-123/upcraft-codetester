...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-agenthorsy", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installAgenthorsy(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-agenthorsy", unavailable)
