...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.agenthorsy.desktop.dev"
}
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"agenthorsy.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // opencode.settings.dat → the electron settings store ("agenthorsy.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "agenthorsy.global.dat").
  const storeName = filename === "opencode.settings.dat" ? "agenthorsy.settings" : filename
