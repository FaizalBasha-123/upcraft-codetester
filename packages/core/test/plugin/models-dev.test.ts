...
import { ModelsDevPlugin } from "@agenthorsy-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.AGENTHORSY_MODELS_PATH,
          disabled: Flag.AGENTHORSY_DISABLE_MODELS_FETCH,
        }
        Flag.AGENTHORSY_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.AGENTHORSY_MODELS_PATH = previous.path
          Flag.AGENTHORSY_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
