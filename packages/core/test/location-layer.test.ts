...
import { define } from "@agenthorsy-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@agenthorsy-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "agenthorsy.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "agenthorsy.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
