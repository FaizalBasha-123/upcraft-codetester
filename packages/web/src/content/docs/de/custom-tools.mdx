...
description: Erstellen Sie Tools, die der LLM in AgentHorsy aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von AgentHorsy wie `read`, `write` und `bash`.
...
- Oder global, indem Sie sie in `~/.config/agenthorsy/tools/` platzieren.
