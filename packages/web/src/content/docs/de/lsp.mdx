---
title: LSP Server
description: AgentHorsy lässt sich in Ihren LSP-Server integrieren.
---

AgentHorsy kann sich in Language Server Protocol (LSP)-Server integrieren, um Diagnosen als Feedback für den Agenten zu verwenden.

---

## Integriert

AgentHorsy verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
Wenn LSP aktiviert ist und AgentHorsy eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer AgentHorsy-Konfiguration aktivieren und anpassen.
...
```json title="agenthorsy.json" {5-7}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="agenthorsy.json" {5-9}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "lsp": {
    "typescript": {
