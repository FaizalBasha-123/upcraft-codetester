---
title: Configuración
description: Usando la configuración AgentHorsy JSON.
---

Puede configurar AgentHorsy usando un archivo de configuración JSON.

---

## Formato

AgentHorsy admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
1. **Configuración remota** (de `.well-known/agenthorsy`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/agenthorsy/agenthorsy.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`agenthorsy.json` en el proyecto): configuración específica del proyecto
...
:::note
Los directorios `.opencode` y `~/.config/agenthorsy` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/agenthorsy`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global AgentHorsy en `~/.config/agenthorsy/agenthorsy.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `agenthorsy.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia AgentHorsy, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
El archivo de configuración tiene un esquema definido en [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json).
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor AgentHorsy.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración AgentHorsy a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, AgentHorsy intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración AgentHorsy a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/agenthorsy/agents/` o `.opencode/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, AgentHorsy recurrirá a `"build"` con una advertencia.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/agenthorsy/commands/` o `.opencode/commands/`. [Más información aquí](/docs/commands).
...
AgentHorsy descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían AgentHorsy con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.opencode/plugins/` o `~/.config/agenthorsy/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir AgentHorsy para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
