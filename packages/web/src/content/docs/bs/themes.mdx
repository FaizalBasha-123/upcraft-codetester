...
U AgentHorsy mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, AgentHorsy koristi nasu `opencode` temu.
...
AgentHorsy dolazi sa vise ugradenih tema.
...
- Zele da AgentHorsy odgovara izgledu njihovog terminala
...
AgentHorsy podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/agenthorsy/themes/*.json` ili `$XDG_CONFIG_HOME/agenthorsy/themes/*.json`
