...
Da biste otklonili probleme s AgentHorsy, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **macOS/Linux**: `~/.local/share/agenthorsy/log/`
...
AgentHorsy pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/agenthorsy/`
...
AgentHorsy Desktop pokreće lokalni AgentHorsy server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `AgentHorsy` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/agenthorsy/agenthorsy.jsonc` (ili `~/.config/agenthorsy/agenthorsy.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
AgentHorsy također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/agenthorsy/plugins/`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga AgentHorsy mogao ponovo izgraditi.

1. Potpuno zatvorite AgentHorsy Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/agenthorsy`
- **Linux**: obrišite `~/.cache/agenthorsy` (ili pokrenite `rm -rf ~/.cache/agenthorsy`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\opencode`

3. Ponovo pokrenite AgentHorsy Desktop.

---

### Rješavanje problema sa vezom na serveru

AgentHorsy Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `agenthorsy.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, AgentHorsy Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa AgentHorsy karakteristikama.

---

### Obavještenja se ne prikazuju

AgentHorsy Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za AgentHorsy u postavkama vašeg OS-a, i
...
1. Zatvorite AgentHorsy Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka AgentHorsy Desktop aplikacije):
...
2. **Pridružite se našem Discordu**
   ...
   [**agenthorsy.ai/discord**](https://agenthorsy.ai/discord)
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. AgentHorsy dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
```bash
   rm -rf ~/.cache/agenthorsy
```

Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\opencode`

2. Ponovo pokrenite AgentHorsy da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti AgentHorsy da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
AgentHorsy će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
