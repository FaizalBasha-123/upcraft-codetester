---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en AgentHorsy.
---

import config from "../../../../config.mjs"
export const console = config.console

AgentHorsy utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration AgentHorsy.
...
en `~/.local/share/agenthorsy/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre AgentHorsy
configuration.
...
AgentHorsy Zen est une liste de modèles fournis par l'équipe AgentHorsy qui ont été
testé et vérifié pour fonctionner correctement avec AgentHorsy. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par AgentHorsy Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez opencode et dirigez-vous vers [agenthorsy.ai/auth](https://agenthorsy.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans AgentHorsy et son utilisation est totalement facultative.

---

## AgentHorsy Go

AgentHorsy Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe AgentHorsy qui ont été
testé et vérifié pour fonctionner correctement avec AgentHorsy.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `AgentHorsy Go` et rendez-vous sur [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
Pour utiliser Amazon Bedrock avec AgentHorsy :
...
Pour une configuration spécifique au projet ou persistante, utilisez `agenthorsy.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans AgentHorsy n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
AgentHorsy utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller AgentHorsy
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `agenthorsy.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth).

##### Configuration

Personnalisez via `agenthorsy.json` :
...
Pour utiliser Google Vertex AI avec AgentHorsy :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) pour enregistrer automatiquement chaque conversation AgentHorsy en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation AgentHorsy répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour AgentHorsy. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/agenthorsy) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec AgentHorsy :
...
3. Copiez la clé API à utiliser dans AgentHorsy.
...
6. **Important** : Avant d'utiliser des modèles cloud dans AgentHorsy, vous devez extraire les informations du modèle localement :
...
AgentHorsy Zen est une liste de modèles testés et vérifiés fournie par l'équipe AgentHorsy. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>AgentHorsy Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **AgentHorsy Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé AgentHorsy API.
...
4. Créez ou mettez à jour votre fichier `agenthorsy.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à AgentHorsy de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
