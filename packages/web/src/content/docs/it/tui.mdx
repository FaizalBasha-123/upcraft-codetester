---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di AgentHorsy.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AgentHorsy fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire AgentHorsy avvia la TUI per la directory corrente.
...
Quando usi la TUI di AgentHorsy, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a AgentHorsy. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da AgentHorsy. _Alias_: `/quit`, `/q`
...
Questo è separato da `agenthorsy.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo AgentHorsy attende dopo il leader key. Default: `2000`.
