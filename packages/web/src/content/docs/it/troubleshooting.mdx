...
Per diagnosticare problemi con AgentHorsy, inizia controllando i log e i dati locali che salva su disco.
...
AgentHorsy Desktop esegue in background un server locale di AgentHorsy (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `AgentHorsy` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
AgentHorsy puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che AgentHorsy possa ricostruirla.

1. Chiudi completamente AgentHorsy Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/agenthorsy`
- **Linux**: elimina `~/.cache/agenthorsy` (oppure esegui `rm -rf ~/.cache/agenthorsy`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\opencode`

3. Riavvia AgentHorsy Desktop.

---

### Risolvi problemi di connessione al server

AgentHorsy Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `agenthorsy.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, AgentHorsy Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di AgentHorsy.

---

### Notifiche non visualizzate

AgentHorsy Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per AgentHorsy nelle impostazioni del sistema operativo, e
...
1. Chiudi AgentHorsy Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app AgentHorsy Desktop):
...
Se riscontri problemi con AgentHorsy:
