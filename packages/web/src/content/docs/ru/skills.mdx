...
- Глобальная конфигурация: `~/.config/agenthorsy/skills/<name>/SKILL.md`.
...
Глобальные определения также загружаются из `~/.config/agenthorsy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `agenthorsy.json`:
...
**Для встроенных агентов** (в формате `agenthorsy.json`):
