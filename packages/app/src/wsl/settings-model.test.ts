...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when AgentHorsy needs attention", () => {
    expect(wslAgenthorsyAction(undefined)).toBeUndefined()
    expect(
      wslAgenthorsyAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install AgentHorsy")
    expect(
      wslAgenthorsyAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/agenthorsy",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update AgentHorsy")
    expect(
      wslAgenthorsyAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/agenthorsy",
  ...
  test("does not report ready when AgentHorsy is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        agenthorsyChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.agenthorsy/bin/agenthorsy",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installAgenthorsy" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-agenthorsy")
  })
