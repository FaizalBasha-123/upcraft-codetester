...
import { type Session } from "@agenthorsy-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("agenthorsy://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("agenthorsy://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("agenthorsy://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("agenthorsy://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("agenthorsy://open-project")).toBeUndefined()
    expect(parseDeepLink("agenthorsy://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "agenthorsy://open-project?directory=/a",
      "agenthorsy://other?directory=/b",
      "agenthorsy://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("agenthorsy://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("agenthorsy://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("agenthorsy://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("agenthorsy://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "agenthorsy://new-session?directory=/a",
      "agenthorsy://open-project?directory=/b",
      "agenthorsy://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __AGENTHORSY__: {
        deepLinks: ["agenthorsy://open-project?directory=/a"],
      },
    } as unknown as Window & { __AGENTHORSY__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["agenthorsy://open-project?directory=/a"])
