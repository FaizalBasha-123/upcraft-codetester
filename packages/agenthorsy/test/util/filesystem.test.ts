...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "agenthorsy.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "agenthorsy.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "agenthorsy.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "agenthorsy.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["agenthorsy.json", "agenthorsy.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "agenthorsy.json"),
        path.join(project, "agenthorsy.jsonc"),
      ])
    })
  })
