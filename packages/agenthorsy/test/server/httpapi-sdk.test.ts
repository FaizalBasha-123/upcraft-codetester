...
import { createOpencodeClient } from "@agenthorsy-ai/sdk/v2"
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.AGENTHORSY_SERVER_PASSWORD = input?.password
      Flag.AGENTHORSY_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".agenthorsy", "skills", "project-rest-skill", "SKILL.md"),
      `---
