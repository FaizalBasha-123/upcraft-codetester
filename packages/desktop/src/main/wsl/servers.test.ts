...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectAgenthorsyVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectAgenthorsyVersion("1.14.35", "1.16.2")).toThrow(
    "AgentHorsy update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating AgentHorsy", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.agenthorsy/bin/agenthorsy",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, agenthorsyChecks: {} })
})
...
test("ignores stale background AgentHorsy checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().agenthorsyChecks).toEqual({})
})

test("ignores stale startup AgentHorsy checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking AgentHorsy", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const agenthorsy: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveAgenthorsy: async (distro) => {
      agenthorsy.push(distro)
      return "/home/me/.agenthorsy/bin/agenthorsy"
    },
  })
  ...
  expect(agenthorsy).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(agenthorsy).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().agenthorsyChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check AgentHorsy in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const agenthorsy: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(agenthorsy).toEqual(["Debian"])
  expect(Object.keys(controller.getState().agenthorsyChecks)).toEqual(["Debian"])
})
