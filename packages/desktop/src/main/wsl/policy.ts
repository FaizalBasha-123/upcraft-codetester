import type { WslDistroProbe, WslAgenthorsyCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  agenthorsyChecks: Record<string, WslAgenthorsyCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextOpencodeChecks = { ...agenthorsyChecks }
  delete nextDistroProbes[distro]
  delete nextOpencodeChecks[distro]
  return { distroProbes: nextDistroProbes, agenthorsyChecks: nextOpencodeChecks }
}
