...
Move the canonical AgentHorsy terminal application from
`packages/agenthorsy/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/agenthorsy ---\
                      > @agenthorsy-ai/tui -> @agenthorsy-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/agenthorsy`,
`packages/cli`, or `@agenthorsy-ai/core`.

The SDK is the TUI's AgentHorsy boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/agenthorsy`.
- Do not replace private `packages/agenthorsy` imports with `@agenthorsy-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@agenthorsy-ai/tui`.

## Ownership Boundary

### `@agenthorsy-ai/tui` Owns
...
- AgentHorsy domain data displayed by the TUI
...
```text
packages/agenthorsy/src/cli/cmd/tui
```

Its private dependency on `packages/agenthorsy` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/agenthorsy/src/*`.
...
Status: Completed. The private `@agenthorsy-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@agenthorsy-ai/tui`.
...
- The package has no dependency on `opencode`, `@agenthorsy-ai/cli`, or
  `@agenthorsy-ai/core`.
...
primitives now live in `@agenthorsy-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@agenthorsy-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads AgentHorsy globals or process state;
...
Replace process-global AgentHorsy state with resolved TUI inputs.
...
`@agenthorsy-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/agenthorsy` initially.
...
canonical in `@agenthorsy-ai/tui`. Configured references resolve through the new
...
adapters rather than AgentHorsy domain implementations.

Make the SDK the only AgentHorsy domain boundary used by the TUI.
...
- Domain-facing TUI code imports AgentHorsy data and operations only from
  `@agenthorsy-ai/sdk`.
...
`@agenthorsy-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/agenthorsy`.
...
- The package has no imports from `packages/agenthorsy`, `packages/cli`, or
  `@agenthorsy-ai/core`.
...
public `@agenthorsy-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/agenthorsy`.
- Keep the legacy embedded worker and server startup in `packages/agenthorsy`.
- Change those adapters to load config, create transport inputs, and call the
  public `@agenthorsy-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/agenthorsy`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@agenthorsy-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@agenthorsy-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for AgentHorsy domain state.
...
cd packages/agenthorsy && bun typecheck
...
rg '@agenthorsy-ai/core|packages/agenthorsy|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/agenthorsy/src packages/agenthorsy/test
```
...
- Build the current-platform `packages/agenthorsy` binary.
