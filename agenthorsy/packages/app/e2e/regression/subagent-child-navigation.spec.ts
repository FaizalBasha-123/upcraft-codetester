...
const directory = "C:/AgentHorsy/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#agenthorsy-titlebar-right")
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem(
        "agenthorsy.global.dat:server",
        JSON.stringify({
          projects: { local: [{ worktree: directory, expanded: true }] },
          lastProject: { local: directory },
        }),
      )
      localStorage.setItem("agenthorsy.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
