...
Det er to typer agenter i AgentHorsy; primære agenter og underagenter.
...
AgentHorsy kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
AgentHorsy kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

AgentHorsy kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `agenthorsy.json`:
...
Hvis ingen temperatur er spesifisert, bruker AgentHorsy modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale AgentHorsy-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i AgentHorsy-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [AgentHorsy Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/agenthorsy).
:::
