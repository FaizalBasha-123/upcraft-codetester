...
Du kan konfigurere AgentHorsy ved å bruke en JSON konfigurasjonsfil.

---

## Format

AgentHorsy støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Ekstern konfigurasjon** (fra `.well-known/agenthorsy`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/agenthorsy/agenthorsy.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`agenthorsy.json` i prosjekt) - prosjektspesifikke innstillinger
...
:::note
`.opencode` og `~/.config/agenthorsy` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/agenthorsy`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale AgentHorsy-konfigurasjon i `~/.config/agenthorsy/agenthorsy.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `agenthorsy.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når AgentHorsy starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Konfigurasjonsfilen har et skjema som er definert i [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json).
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage AgentHorsy-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i AgentHorsy-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver AgentHorsy å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/agenthorsy/agents/` eller `.opencode/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil AgentHorsy falle tilbake til `"build"` med en advarsel.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/agenthorsy/commands/` eller `.opencode/commands/`. [Les mer her](/docs/commands).
...
AgentHorsy vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater AgentHorsy alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider AgentHorsy med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.opencode/plugins/` eller `~/.config/agenthorsy/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense AgentHorsy til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
