...
For å feilsøke problemer med AgentHorsy, start med å sjekke loggene og lokale data den lagrer på disken.
...
AgentHorsy lagrer øktdata og andre applikasjonsdata på disken på:
...
AgentHorsy Desktop kjører en lokal AgentHorsy-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `AgentHorsy`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/agenthorsy/agenthorsy.jsonc` (eller `~/.config/agenthorsy/agenthorsy.json`)
- **macOS/Linux** (eldre installasjoner): `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
AgentHorsy kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at AgentHorsy kan gjenoppbygge den.

1. Avslutt AgentHorsy Desktop helt.
2. Slett hurtigbufferkatalogen:

- **macOS**: Finder -> `Cmd+Shift+G` -> lim inn `~/.cache/agenthorsy`
- **Linux**: slett `~/.cache/agenthorsy` (eller kjør `rm -rf ~/.cache/agenthorsy`)
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.cache\opencode`

3. Start AgentHorsy Desktop på nytt.

---

### Løs problemer med servertilkobling

AgentHorsy Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
Hvis `agenthorsy.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.
...
På Windows krever AgentHorsy Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til AgentHorsy.

---

### Varsler vises ikke

AgentHorsy Desktop viser bare systemvarsler når:

- varsler er aktivert for AgentHorsy i OS-innstillingene dine, og
...
1. Avslutt AgentHorsy Desktop.
2. Finn og slett disse filene (de finnes i AgentHorsy Desktop-appens datakatalog):
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte provider-pakker. AgentHorsy installerer dynamisk provider-pakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
2. Start AgentHorsy på nytt for å installere de nyeste provider-pakkene på nytt

Dette vil tvinge AgentHorsy til å laste ned de nyeste versjonene av provider-pakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
AgentHorsy vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavle-verktøy i rekkefølgen: `xclip` og `xsel`.
