...
description: Skriv dine egne programtillegg for å utvide AgentHorsy.
---

Plugins lar deg utvide AgentHorsy ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til AgentHorsy.
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/agenthorsy/node_modules/`.
...
1. Global konfigurasjon (`~/.config/agenthorsy/agenthorsy.json`)
2. Prosjektkonfigurasjon (`agenthorsy.json`)
3. Global plugin-katalog (`~/.config/agenthorsy/plugins/`)
...
AgentHorsy kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En AgentHorsy SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide AgentHorsy.
...
:::note
Hvis du bruker AgentHorsy-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre AgentHorsy fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til AgentHorsy:
...
`tool`-hjelperen lager et tilpasset verktøy som AgentHorsy kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for AgentHorsy sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://agenthorsy.ai/docs/sdk) for detaljer.
