...
Du kan legge til eksterne verktøy i AgentHorsy ved å bruke _Model Context Protocol_ eller MCP. AgentHorsy støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [AgentHorsy Config](https://agenthorsy.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="agenthorsy.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/agenthorsy`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="agenthorsy.jsonc" {15}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
AgentHorsy håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil AgentHorsy:
...
Hvis serveren krever autentisering, vil AgentHorsy be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opencode mcp auth <server-name>`.
...
```json title="agenthorsy.json" {7-11}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil AgentHorsy lagre tokens sikkert i `~/.local/share/agenthorsy/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i AgentHorsy, sammen med innebygde verktøy. Så du kan administrere dem gjennom AgentHorsy-konfigurasjonen som alle andre verktøy.
...
```json title="agenthorsy.json" {11, 14-18}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="agenthorsy.json" {4-8}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "mcp": {
    "sentry": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble AgentHorsy til Sentry-kontoen din.
...
```json title="agenthorsy.json" {4-7}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="agenthorsy.json" {7-9}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver AgentHorsy-samtale oppført som en separat økt.
