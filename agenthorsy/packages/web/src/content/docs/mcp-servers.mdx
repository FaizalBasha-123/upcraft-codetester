...
You can add external tools to AgentHorsy using the _Model Context Protocol_, or MCP. AgentHorsy supports both local and remote servers.
...
You can define MCP servers in your [AgentHorsy Config](https://agenthorsy.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/agenthorsy` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
AgentHorsy automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, AgentHorsy will:
...
If the server requires authentication, AgentHorsy will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, AgentHorsy will store the tokens securely in `~/.local/share/agenthorsy/mcp-auth.json`.
...
Your MCPs are available as tools in AgentHorsy, alongside built-in tools. So you can manage them through the AgentHorsy config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect AgentHorsy to your Sentry account.
