---
title: Proveedores
description: Usando cualquier proveedor LLM en AgentHorsy.
---

import config from "../../../../config.mjs"
export const console = config.console

AgentHorsy usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración AgentHorsy.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu AgentHorsy
configuración.
...
AgentHorsy Zen es una lista de modelos proporcionados por el equipo AgentHorsy que han sido
probado y verificado para funcionar bien con AgentHorsy. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con AgentHorsy Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione opencode y diríjase a [agenthorsy.ai/auth](https://agenthorsy.ai/auth).
...
Funciona como cualquier otro proveedor en AgentHorsy y su uso es completamente opcional.

---

## AgentHorsy Go

AgentHorsy Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de AgentHorsy que han sido
probado y verificado para funcionar bien con AgentHorsy.

1. Ejecute el comando `/connect` en TUI, seleccione `AgentHorsy Go` y diríjase a [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
Para usar Amazon Bedrock con AgentHorsy:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `agenthorsy.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en AgentHorsy no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `AgentHorsy`, Alcances: `api`
...
AgentHorsy utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear AgentHorsy
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `agenthorsy.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth).

##### Configuración

Personalizar a través de `agenthorsy.json`:
...
Para utilizar Google Vertex AI con AgentHorsy:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) para registrar automáticamente cada conversación AgentHorsy como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación AgentHorsy enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para AgentHorsy. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/agenthorsy) para obtener más detalles.
:::
...
Para usar Ollama Cloud con AgentHorsy:
...
3. Copie la clave API para usarla en AgentHorsy.
...
6. **Importante**: Antes de usar modelos en la nube en AgentHorsy, debe obtener la información del modelo localmente:
...
AgentHorsy Zen es una lista de modelos probados y verificados proporcionada por el equipo AgentHorsy. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>AgentHorsy Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **AgentHorsy Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave AgentHorsy API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con AgentHorsy:
...
4. Cree o actualice su archivo `agenthorsy.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a AgentHorsy comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
