...
Con AgentHorsy puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, AgentHorsy usa nuestro propio tema `opencode`.
...
AgentHorsy viene con varios temas integrados.
...
- Quiere que AgentHorsy coincida con la apariencia de su terminal
...
AgentHorsy admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/agenthorsy/themes/*.json` o `$XDG_CONFIG_HOME/agenthorsy/themes/*.json`
