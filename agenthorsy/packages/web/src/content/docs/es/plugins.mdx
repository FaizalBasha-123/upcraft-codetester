---
title: Complementos
description: Escriba sus propios complementos para extender AgentHorsy.
---

Los complementos le permiten extender AgentHorsy al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de AgentHorsy.
...
- `~/.config/agenthorsy/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/agenthorsy/node_modules/`.
...
1. Configuración global (`~/.config/agenthorsy/agenthorsy.json`)
2. Configuración del proyecto (`agenthorsy.json`)
3. Directorio global de complementos (`~/.config/agenthorsy/plugins/`)
...
AgentHorsy ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
:::note
Si está utilizando la aplicación de escritorio AgentHorsy, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://agenthorsy.ai/docs/sdk) para obtener más detalles.
