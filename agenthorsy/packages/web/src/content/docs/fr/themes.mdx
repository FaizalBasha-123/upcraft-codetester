...
Avec AgentHorsy, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, AgentHorsy utilise notre propre thème `opencode`.
...
AgentHorsy est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que AgentHorsy corresponde à l'apparence de leur terminal
...
AgentHorsy prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/agenthorsy/themes/*.json` ou `$XDG_CONFIG_HOME/agenthorsy/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/agenthorsy/themes
vim ~/.config/agenthorsy/themes/my-theme.json
```
...
{
  "$schema": "https://agenthorsy.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
