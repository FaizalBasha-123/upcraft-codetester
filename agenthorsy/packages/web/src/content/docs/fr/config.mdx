---
title: Configuration
description: Utilisation de la configuration JSON AgentHorsy.
---

Vous pouvez configurer AgentHorsy à l'aide d'un fichier de configuration JSON.

---

## Format

AgentHorsy prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/agenthorsy`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/agenthorsy/agenthorsy.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OPENCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`agenthorsy.json` dans le projet) - paramètres spécifiques au projet
...
:::note
Les répertoires `.opencode` et `~/.config/agenthorsy` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/agenthorsy`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale AgentHorsy dans `~/.config/agenthorsy/agenthorsy.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/agenthorsy/tui.json`.
...
Ajoutez `agenthorsy.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque AgentHorsy démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Le fichier de configuration a un schéma défini dans [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json).

La configuration TUI utilise [**`agenthorsy.ai/tui.json`**](https://agenthorsy.ai/tui.json).
...
Les anciennes clés `theme`, `keybinds` et `tui` dans `agenthorsy.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur AgentHorsy.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration AgentHorsy via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, AgentHorsy essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/agenthorsy/agents/` ou `.opencode/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, AgentHorsy reviendra à `"build"` avec un avertissement.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/agenthorsy/commands/` ou `.opencode/commands/`. [En savoir plus ici](/docs/commands).
...
AgentHorsy téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
[Plugins](/docs/plugins) étendent AgentHorsy avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.opencode/plugins/` ou `~/.config/agenthorsy/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre AgentHorsy à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
