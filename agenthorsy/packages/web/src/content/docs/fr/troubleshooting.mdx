...
Pour déboguer les problèmes avec AgentHorsy, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
AgentHorsy Desktop exécute un serveur AgentHorsy local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `AgentHorsy` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/agenthorsy/agenthorsy.jsonc` (ou `~/.config/agenthorsy/agenthorsy.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
AgentHorsy peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que AgentHorsy puisse le reconstruire.

1. Quittez complètement AgentHorsy Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/agenthorsy`
- **Linux** : supprimez `~/.cache/agenthorsy` (ou exécutez `rm -rf ~/.cache/agenthorsy`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\opencode`

3. Redémarrez le bureau AgentHorsy.

---

### Résoudre les problèmes de connexion au serveur

AgentHorsy Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `agenthorsy.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, AgentHorsy Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de AgentHorsy.

---

### Les notifications ne s'affichent pas

AgentHorsy Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour AgentHorsy dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau AgentHorsy.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application AgentHorsy Desktop) :
...
Si vous rencontrez des problèmes avec AgentHorsy :
