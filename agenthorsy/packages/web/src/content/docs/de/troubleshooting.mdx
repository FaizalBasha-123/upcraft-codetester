...
Wenn AgentHorsy Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
AgentHorsy Desktop startet im Hintergrund einen lokalen AgentHorsy-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `AgentHorsy`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/agenthorsy/agenthorsy.jsonc` (oder `~/.config/agenthorsy/agenthorsy.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
AgentHorsy kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit AgentHorsy ihn neu aufbauen kann.

1. Beende AgentHorsy Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/agenthorsy`
- **Linux**: loesche `~/.cache/agenthorsy` (oder fuehre aus: `rm -rf ~/.cache/agenthorsy`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\opencode`

3. Starte AgentHorsy Desktop neu.

---

### Server-Verbindungsprobleme beheben

AgentHorsy Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `agenthorsy.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt AgentHorsy Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit AgentHorsy-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

AgentHorsy Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer AgentHorsy in den OS-Einstellungen aktiviert sind, und
...
1. Beende AgentHorsy Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von AgentHorsy Desktop):
...
Wenn du Probleme mit AgentHorsy hast:
