---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um AgentHorsy zu erweitern.
---

Mit Plugins können Sie AgentHorsy erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von AgentHorsy zu ändern.
...
- `~/.config/agenthorsy/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/agenthorsy/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/agenthorsy/agenthorsy.json`)
2. Projektkonfiguration (`agenthorsy.json`)
3. Globales Plugin-Verzeichnis (`~/.config/agenthorsy/plugins/`)
...
AgentHorsy führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein AgentHorsy-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie AgentHorsy erweitern können.
...
:::note
Wenn Sie die AgentHorsy-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass AgentHorsy `.env`-Dateien liest:
...
Plugins können AgentHorsy auch benutzerdefinierte Tools hinzufügen:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@agenthorsy-ai/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das AgentHorsy aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für AgentHorsy zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://agenthorsy.ai/docs/sdk).
