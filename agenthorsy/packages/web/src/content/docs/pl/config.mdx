---
title: Konfiguracja
description: Ustawienia AgentHorsy w pliku JSON.
---

Możesz dostosować AgentHorsy za pomocą pliku konfiguracyjnego JSON.

---

## Format

AgentHorsy obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
1. **Konfiguracja zdalna** (z `.well-known/agenthorsy`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/agenthorsy/agenthorsy.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`agenthorsy.json` w projekcie) - ustawienia specyficzne dla projektu
...
:::note
W katalogach `.opencode` i `~/.config/agenthorsy` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/agenthorsy`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację AgentHorsy w `~/.config/agenthorsy/agenthorsy.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/agenthorsy/tui.json`.
...
Dodaj `agenthorsy.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy AgentHorsy się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json).
...
Przestarzałe klucze `theme`, `keybinds` i `tui` w `agenthorsy.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer AgentHorsy.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie AgentHorsy wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/agenthorsy/agents/` lub `.opencode/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, AgentHorsy powróci do `"build"` z ostrzeżeniem.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/agenthorsy/commands/` lub `.opencode/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
AgentHorsy może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy AgentHorsy nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie AgentHorsy **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają AgentHorsy o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.opencode/plugins/` lub `~/.config/agenthorsy/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie AgentHorsy tylko do określonych dostawców.
