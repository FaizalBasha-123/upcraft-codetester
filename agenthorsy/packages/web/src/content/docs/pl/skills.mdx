...
Ładowane są także definicje globalne z `~/.config/agenthorsy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `agenthorsy.json`:
...
**Dla agentów wbudowanych** (w `agenthorsy.json`):
