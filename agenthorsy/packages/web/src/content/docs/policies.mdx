---
title: Policies
description: Control which configured resources AgentHorsy may use.
---

Policies control whether AgentHorsy may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `agenthorsy.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether AgentHorsy may use a resource such as an LLM provider.
...
AgentHorsy currently supports one policy action:
