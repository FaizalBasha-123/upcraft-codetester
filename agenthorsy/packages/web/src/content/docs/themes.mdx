...
With AgentHorsy you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, AgentHorsy uses our own `opencode` theme.
...
AgentHorsy comes with several built-in themes.
...
- Want AgentHorsy to match their terminal's appearance
...
AgentHorsy supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/agenthorsy/themes/*.json` or `$XDG_CONFIG_HOME/agenthorsy/themes/*.json`
