---
title: Dodaci
description: Napišite vlastite dodatke za proširenje AgentHorsy.
---

Dodaci vam omogućavaju da proširite AgentHorsy spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja AgentHorsy.
...
- `~/.config/agenthorsy/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/agenthorsy/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/agenthorsy/agenthorsy.json`)
2. Konfiguracija projekta (`agenthorsy.json`)
3. Globalni direktorij dodataka (`~/.config/agenthorsy/plugins/`)
...
AgentHorsy pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje AgentHorsy.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://agenthorsy.ai/docs/sdk) za detalje.
