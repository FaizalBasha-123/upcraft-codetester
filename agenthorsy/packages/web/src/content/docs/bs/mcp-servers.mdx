...
Možete dodati vanjske alate u AgentHorsy koristeći _Model Context Protocol_, ili MCP. AgentHorsy podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [AgentHorsy Config](https://agenthorsy.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/agenthorsy`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
AgentHorsy automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, AgentHorsy će:
...
Ako server zahtijeva autentifikaciju, AgentHorsy će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, AgentHorsy sigurno čuva tokene u `~/.local/share/agenthorsy/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u AgentHorsy, zajedno s ugrađenim alatima. Možete njima upravljati kroz AgentHorsy konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete AgentHorsy sa vašim Sentry nalogom.
