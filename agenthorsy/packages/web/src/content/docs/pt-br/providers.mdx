...
Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/agenthorsy/auth.json`.
...
AgentHorsy Zen é uma lista de modelos fornecidos pela equipe do opencode que foram testados e verificados para funcionar bem com o opencode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o AgentHorsy Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `AgentHorsy Zen` e acesse [agenthorsy.ai/zen](https://agenthorsy.ai/zen).
...
AgentHorsy Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do opencode que foram
testados e verificados para funcionar bem com o opencode.

1. Execute o comando `/connect` no TUI, selecione `AgentHorsy Go` e acesse [agenthorsy.ai/zen](https://agenthorsy.ai/zen).
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `agenthorsy.json`:
...
arquivo `agenthorsy.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth).

##### Configuração

Personalize através do `agenthorsy.json`:
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) para registrar automaticamente cada conversa do opencode como uma sessão no Helicone.
...
:::tip
Ollama pode se configurar automaticamente para o opencode. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/agenthorsy) para detalhes.
:::
...
AgentHorsy Zen é uma lista de modelos testados e verificados fornecidos pela equipe do opencode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>AgentHorsy Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **AgentHorsy Zen**.
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no agenthorsy.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `agenthorsy.json` no diretório do seu projeto:
