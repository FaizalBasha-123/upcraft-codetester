...
- `~/.config/agenthorsy/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/agenthorsy/node_modules/`.
...
1. Configuração global (`~/.config/agenthorsy/agenthorsy.json`)
2. Configuração do projeto (`agenthorsy.json`)
3. Diretório de plugins global (`~/.config/agenthorsy/plugins/`)
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://agenthorsy.ai/docs/sdk) para detalhes.
