---
title: Plugins
description: Skriv dine egne plugins for at udvide AgentHorsy.
---

Plugins giver dig mulighed for at udvide AgentHorsy ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre AgentHorsys standardadfærd.
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/agenthorsy/node_modules/`.
...
1. Global konfiguration (`~/.config/agenthorsy/agenthorsy.json`)
2. Projektkonfiguration (`agenthorsy.json`)
3. Global plugin-mappe (`~/.config/agenthorsy/plugins/`)
...
AgentHorsy kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
:::note
Hvis du bruger AgentHorsy desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Dine tilpassede værktøjer vil være tilgængelige for agenthorsy sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://agenthorsy.ai/docs/sdk) for detaljer.
