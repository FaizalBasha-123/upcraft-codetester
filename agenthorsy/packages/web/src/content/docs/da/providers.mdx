---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i AgentHorsy.
---

import config from "../../../../config.mjs"
export const console = config.console

AgentHorsy bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i AgentHorsy-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/agenthorsy/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i AgentHorsy-konfigurationen.
...
AgentHorsy Zen er en liste over modeller leveret af AgentHorsy-teamet, som er blevet testet og verificeret til at fungere godt med AgentHorsy. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med AgentHorsy Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg opencode og gå til [agenthorsy.ai/auth](https://agenthorsy.ai/auth).
...
Det fungerer som alle andre udbydere i AgentHorsy og er helt valgfrit at bruge.

---

## AgentHorsy Go

AgentHorsy Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af AgentHorsy-teamet, som er testet og verificeret til at fungere godt med AgentHorsy.

1. Kør kommandoen `/connect` i TUI, vælg `AgentHorsy Go`, og gå til [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
Sådan bruger du Amazon Bedrock med AgentHorsy:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `agenthorsy.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i AgentHorsy understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
AgentHorsy bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse AgentHorsy
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`agenthorsy.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `agenthorsy.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med AgentHorsy:
...
Sådan bruger du Google Vertex AI med AgentHorsy:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) plugin for automatisk at logge hver AgentHorsy-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver AgentHorsy-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for AgentHorsy. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/agenthorsy) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med AgentHorsy:
...
3. Kopier API-nøglen til brug i AgentHorsy.
...
6. **Vigtigt**: Før du bruger skymodeller i AgentHorsy, skal du hente modelinformationen lokalt:
...
AgentHorsy Zen er en liste over testede og verificerede modeller leveret af AgentHorsy-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>AgentHorsy Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **AgentHorsy Zen**.

   ```txt
   /connect
   ```

3. Indtast AgentHorsy API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AgentHorsy:
...
4. Opret eller opdater `agenthorsy.json`-filen i projektkataloget:
...
`limit`-felterne lader AgentHorsy forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
