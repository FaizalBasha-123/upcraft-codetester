...
Med AgentHorsy kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger AgentHorsy vårt eget `opencode`-tema.
...
AgentHorsy kommer med flere innebygde temaer.
...
- Vil at AgentHorsy skal matche terminalens utseende
...
AgentHorsy støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/agenthorsy/themes/*.json` eller `$XDG_CONFIG_HOME/agenthorsy/themes/*.json`
