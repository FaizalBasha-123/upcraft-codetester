---
title: Konfiguration
description: Ved at bruge AgentHorsy JSON konfig.
---

Du kan konfigurere AgentHorsy ved hjælp af en JSON-konfigurationsfil.

---

## Format

AgentHorsy understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/agenthorsy`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/agenthorsy/agenthorsy.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`agenthorsy.json` i projekt) - projektspecifikke indstillinger
...
:::note
`.opencode` og `~/.config/agenthorsy` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/agenthorsy`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale AgentHorsy-konfiguration i `~/.config/agenthorsy/agenthorsy.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `agenthorsy.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når AgentHorsy starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Konfigurationsfilen har et skema, der defineres i [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json).
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din AgentHorsy-server.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din AgentHorsy-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger AgentHorsy at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din AgentHorsy-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/agenthorsy/agents/` eller `.opencode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil AgentHorsy falde tilbage til `"build"` med en advarsel.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/agenthorsy/commands/` eller `.opencode/commands/`. [Learn more here](/docs/commands).
...
AgentHorsy vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
[Plugins](/docs/plugins) udvide AgentHorsy med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.opencode/plugins/` eller `~/.config/agenthorsy/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse AgentHorsy til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
