...
For at fejlfinde problemer med AgentHorsy, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
AgentHorsy Desktop kører en lokal AgentHorsy-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `AgentHorsy`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
AgentHorsy kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så AgentHorsy kan genopbygge den.

1. Afslut AgentHorsy Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/agenthorsy`
- **Linux**: slet `~/.cache/agenthorsy` (eller kør `rm -rf ~/.cache/agenthorsy`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\opencode`

3. Start AgentHorsy Desktop på ny.

---

### Løs problemer med serverforbindelse

AgentHorsy Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `agenthorsy.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver AgentHorsy Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i AgentHorsy.

---

### Meddelelser vises ikke

AgentHorsy Desktop viser kun systemvarsler når:

- varsler er aktiveret for AgentHorsy i dine OS-indstillinger, og
...
1. Afslut AgentHorsy Desktop.
2. Find og slet disse filer (de findes i AgentHorsy Desktop-appens datamappe):
...
Hvis du oplever problemer med AgentHorsy:
