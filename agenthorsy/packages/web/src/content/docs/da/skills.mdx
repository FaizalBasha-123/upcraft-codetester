...
Agentferdigheter lar AgentHorsy oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
AgentHorsy søker etter disse stedene:

- Prosjektkonfigurasjon: `.opencode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/agenthorsy/skills/<name>/SKILL.md`
...
For prosjektlokale stier går AgentHorsy opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
...
Globale definisjoner lastes også inn fra `~/.config/agenthorsy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
AgentHorsy viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `agenthorsy.json`:
...
**For innebygde agenter** (i `agenthorsy.json`):
