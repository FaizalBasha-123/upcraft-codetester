...
To debug issues with AgentHorsy, start by checking the logs and local data it stores on disk.
...
To uninstall the AgentHorsy CLI and remove its related files, run:
...
To remove AgentHorsy Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

AgentHorsy Desktop runs a local AgentHorsy server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `AgentHorsy` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/agenthorsy/agenthorsy.jsonc` (or `~/.config/agenthorsy/agenthorsy.json`)
- **macOS/Linux** (older installs): `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
AgentHorsy can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so AgentHorsy can rebuild it.

1. Quit AgentHorsy Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/agenthorsy`
- **Linux**: delete `~/.cache/agenthorsy` (or run `rm -rf ~/.cache/agenthorsy`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\opencode`

3. Restart AgentHorsy Desktop.

---

### Fix server connection issues

AgentHorsy Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `agenthorsy.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, AgentHorsy Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with AgentHorsy's features.

---

### Notifications not showing

AgentHorsy Desktop only shows system notifications when:

- notifications are enabled for AgentHorsy in your OS settings, and
...
1. Quit AgentHorsy Desktop.
2. Find and delete these files (they live in the AgentHorsy Desktop app data directory):
...
If you're experiencing issues with AgentHorsy:
