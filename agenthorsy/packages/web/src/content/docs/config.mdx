---
title: Config
description: Using the AgentHorsy JSON config.
---

You can configure AgentHorsy using a JSON config file.

---

## Format

AgentHorsy supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="agenthorsy.jsonc"
{
...
1. **Remote config** (from `.well-known/agenthorsy`) - organizational defaults
2. **Global config** (`~/.config/agenthorsy/agenthorsy.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`agenthorsy.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/agenthorsy/` on macOS) - admin-controlled
...
:::note
The `.opencode` and `~/.config/agenthorsy` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/agenthorsy` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/agenthorsy"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global AgentHorsy config in `~/.config/agenthorsy/agenthorsy.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/agenthorsy/tui.json`.
...
Add `agenthorsy.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When AgentHorsy starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Drop an `agenthorsy.json` or `agenthorsy.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/agenthorsy/` |
| Linux    | `/etc/agenthorsy/`                         |
...
On macOS, AgentHorsy reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

AgentHorsy checks these paths:

1. `/Library/Managed Preferences/<user>/ai.opencode.managed.plist`
2. `/Library/Managed Preferences/ai.opencode.managed.plist`

The plist keys map directly to `agenthorsy.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.opencode.managed` PayloadType. The AgentHorsy config keys go directly in the payload dict:
...
The server/runtime config schema is defined in [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json).

TUI config uses [**`agenthorsy.ai/tui.json`**](https://agenthorsy.ai/tui.json).
...
Legacy `theme`, `keybinds`, and `tui` keys in `agenthorsy.json` are deprecated and automatically migrated when possible.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your AgentHorsy server.
...
If not specified, AgentHorsy will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your AgentHorsy config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, AgentHorsy tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny AgentHorsy actions on configured resources. Currently, policies can control which providers AgentHorsy may use.
...
AgentHorsy normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, AgentHorsy omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/agenthorsy/agents/` or `.opencode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, AgentHorsy will fall back to `"build"` with a warning.
...
You can also define commands using markdown files in `~/.config/agenthorsy/commands/` or `.opencode/commands/`. [Learn more here](/docs/commands).
...
AgentHorsy uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
AgentHorsy will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend AgentHorsy with custom tools, hooks, and integrations.

Place plugin files in `.opencode/plugins/` or `~/.config/agenthorsy/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict AgentHorsy to only use specific providers rather than disabling them one by one.
