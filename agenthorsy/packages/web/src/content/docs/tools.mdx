...
Tools allow the LLM to perform actions in your codebase. AgentHorsy comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in AgentHorsy.
...
:::note
This tool is only available when using the AgentHorsy provider or when the `OPENCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching AgentHorsy:

```bash
OPENCODE_ENABLE_EXA=1 agenthorsy
```
...
AgentHorsy includes specialized tools for autonomous task execution, used primarily by the orchestrator and dynamic_persona agent.

---

### validate_task

Validates that a completed task matches its original plan. Runs AST-level checks, verifies acceptance criteria, and compares implementation against the plan's verification steps.

**When it runs:** Automatically after all todos are completed in build mode, before merging.

**Output:** Pass/fail with detailed discrepancies if the implementation doesn't match the plan.

---

### merge_worktree

Safely merges the agent's worktree back into the main codebase. Only runs after `validate_task` passes.

**When it runs:** Automatically after validation passes.

**Output:** Confirmation of merge with file change summary.

---

### switch_mode

Transitions the dynamic_persona agent from plan mode to build mode. Injected automatically when the agent calls `plan_exit` or `switch_mode`.

**When it runs:** After the planning phase is complete and the agent is ready to execute.

---

### plan_exit

Signals that the planning phase is complete. Triggers the transition to build mode with the plan file as context.

**When it runs:** When the dynamic_persona agent finishes creating its plan.
