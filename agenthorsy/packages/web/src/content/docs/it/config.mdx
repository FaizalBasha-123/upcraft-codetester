...
description: Usa la configurazione JSON di AgentHorsy.
---

Puoi configurare AgentHorsy usando un file di configurazione JSON.

---

## Formato

AgentHorsy supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/agenthorsy`) - default dell'organizzazione
2. **Config globale** (`~/.config/agenthorsy/agenthorsy.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OPENCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`agenthorsy.json` nel progetto) - impostazioni specifiche del progetto
...
:::note
Le directory `.opencode` e `~/.config/agenthorsy` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/agenthorsy`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di AgentHorsy in `~/.config/agenthorsy/agenthorsy.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/agenthorsy/tui.json`.
...
Aggiungi `agenthorsy.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando AgentHorsy si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Il file di configurazione ha uno schema definito in [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json).

La configurazione TUI usa [**`agenthorsy.ai/tui.json`**](https://agenthorsy.ai/tui.json).
...
Le chiavi legacy `theme`, `keybinds` e `tui` in `agenthorsy.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server AgentHorsy.
...
Puoi configurare provider e modelli da usare in AgentHorsy tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, AgentHorsy prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/agenthorsy/agents/` o `.opencode/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, AgentHorsy fara fallback su `"build"` mostrando un avviso.
...
Puoi anche definire comandi usando file markdown in `~/.config/agenthorsy/commands/` o `.opencode/commands/`. [Scopri di piu](/docs/commands).
...
AgentHorsy scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono AgentHorsy con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.opencode/plugins/` o `~/.config/agenthorsy/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare AgentHorsy a usare solo alcuni provider invece di disabilitarli uno per uno.
