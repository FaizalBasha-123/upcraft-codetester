...
تتيح مهارات الوكيل لـ AgentHorsy اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث AgentHorsy في هذه المواقع:

- إعدادات المشروع: `.opencode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/agenthorsy/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج AgentHorsy صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
...
كما تُحمَّل التعريفات العامة أيضاً من `~/.config/agenthorsy/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
يعرض AgentHorsy المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `agenthorsy.json`:
...
**للوكلاء المدمجين** (في `agenthorsy.json`):
