---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع AgentHorsy.
---

تتيح لك الإضافات توسيع AgentHorsy عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ AgentHorsy.
...
- `~/.config/agenthorsy/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/agenthorsy/node_modules/`.
...
1. إعدادات عامة (`~/.config/agenthorsy/agenthorsy.json`)
2. إعدادات المشروع (`agenthorsy.json`)
3. دليل الإضافات العام (`~/.config/agenthorsy/plugins/`)
...
يشغّل AgentHorsy الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
:::note
إذا كنت تستخدم تطبيق AgentHorsy لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://agenthorsy.ai/docs/sdk) للتفاصيل.
