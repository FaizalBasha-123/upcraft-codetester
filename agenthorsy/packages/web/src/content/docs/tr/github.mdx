...
opencode, GitHub iş akışınızla bütünleşir. Yorumunuzda `/agenthorsy` veya `/oc`'den bahsedin; opencode, GitHub Actions çalıştırıcınızdaki görevleri yürütecektir.
...
1. **GitHub uygulamasını yükleyin**
   ...
   [**github.com/apps/agenthorsy-agent**](https://github.com/apps/agenthorsy-agent)'a gidin. Hedef depoya kurulu olduğundan emin olun.

2. **İş akışını ekleyin**

   Aşağıdaki iş akışı dosyasını deponuzdaki `.github/workflows/agenthorsy.yml`'a ekleyin. `model`'de uygun `env` ve gerekli API anahtarlarını ayarladığınızdan emin olun.
...
| ----------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Bir soruna veya halkla ilişkilere yorum yapın | Yorumunuzda `/agenthorsy` veya `/oc`'den bahsedin. opencode bağlamı okur ve şubeler oluşturabilir, PR'leri açabilir veya yanıt verebilir.             |
| `pull_request_review_comment` | PR'deki belirli kod satırlarına yorum yapın   | Kodu incelerken `/agenthorsy` veya `/oc`'den bahsedin. opencode dosya yolunu, satır numaralarını ve fark içeriğini alır.                              |
...
Otomatik görevleri gerçekleştirmek için agenthorsy'u bir zamanlamaya göre çalıştırın:
