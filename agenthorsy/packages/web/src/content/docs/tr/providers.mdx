...
`~/.local/share/agenthorsy/auth.json` içinde.
...
AgentHorsy Zen, opencode ekibi tarafından sağlanan modellerin bir listesidir.
opencode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz AgentHorsy Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, opencode'u seçin ve [agenthorsy.ai/auth](https://agenthorsy.ai/auth)'ye gidin.
...
AgentHorsy Go, opencode ile iyi çalıştığı test edilmiş ve doğrulanmış, opencode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `AgentHorsy Go`'yu seçin ve [agenthorsy.ai/auth](https://agenthorsy.ai/zen) adresine gidin.
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `agenthorsy.json` kullanın:
...
`agenthorsy.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth) ana sayfasında.

##### Yapılandırma

`agenthorsy.json` üzerinden özelleştirin:
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her opencode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) eklentisini kullanın.
...
:::tip
Ollama kendisini opencode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/agenthorsy)'a bakın.
:::
...
AgentHorsy Zen, opencode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>AgentHorsy Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **AgentHorsy Zen**'i arayın.
...
4. Proje dizininizde `agenthorsy.json` dosyanızı oluşturun veya güncelleyin:
...
2. Özel sağlayıcılar için agenthorsy yapılandırmasını kontrol edin ve:
