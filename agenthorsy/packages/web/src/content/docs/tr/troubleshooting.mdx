...
- **macOS/Linux**: `~/.config/agenthorsy/agenthorsy.jsonc` (veya `~/.config/agenthorsy/agenthorsy.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\agenthorsy.jsonc` yapistirin
...
- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/agenthorsy`
- **Linux**: `~/.cache/agenthorsy` dizinini silin (veya `rm -rf ~/.cache/agenthorsy` calistirin)
...
`agenthorsy.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
2. En guncel provider paketlerini yeniden kurmak icin agenthorsy'u yeniden baslatin
