---
title: Eklentiler
description: AgentHorsy'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek AgentHorsy'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya AgentHorsy'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `~/.config/agenthorsy/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/agenthorsy/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/agenthorsy/agenthorsy.json`)
2. Project config (`agenthorsy.json`)
3. Global eklenti dizini (`~/.config/agenthorsy/plugins/`)
...
AgentHorsy bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir AgentHorsy SDK istemcisi.
...
AgentHorsy'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
AgentHorsy masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
AgentHorsy'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca AgentHorsy'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, AgentHorsy'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra AgentHorsy'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://agenthorsy.ai/docs/sdk) bakın.
