...
1. **Uzak yapılandırma** (`.well-known/agenthorsy`'dan) - kurumsal varsayılanlar
2. **Genel yapılandırma** (`~/.config/agenthorsy/agenthorsy.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`OPENCODE_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `agenthorsy.json`) - projeye özgü ayarlar
...
:::note
`.opencode` ve `~/.config/agenthorsy` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Kuruluşlar, `.well-known/agenthorsy` uç noktası aracılığıyla varsayılan yapılandırmayı sağlayabilir. Bu, onu destekleyen bir sağlayıcıyla kimlik doğrulaması yaptığınızda otomatik olarak alınır.
...
Global opencode yapılandırmanızı `~/.config/agenthorsy/agenthorsy.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `agenthorsy.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.
...
Yapılandırma dosyası [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json)'da tanımlanan bir şemaya sahiptir.
...
Agent'ları `~/.config/agenthorsy/agents/` veya `.opencode/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Komutları `~/.config/agenthorsy/commands/` veya `.opencode/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.opencode/plugins/` veya `~/.config/agenthorsy/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
