...
import { PromptInput } from "@agenthorsy-ai/schema/prompt-input"
import { Session } from "@agenthorsy-ai/schema/session"
import { Project } from "@agenthorsy-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@agenthorsy-ai/schema/schema"
...
import { SessionEvent } from "@agenthorsy-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this AgentHorsy process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this AgentHorsy process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
