...
const TUI_SCHEMA_URL = "https://agenthorsy.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from agenthorsy.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const files = await agenthorsyFiles(input)
...
async function agenthorsyFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "agenthorsy"),
    ...(await Filesystem.findUp(["agenthorsy.json", "agenthorsy.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "agenthorsy"))
  }
  if (Flag.AGENTHORSY_CONFIG) files.push(Flag.AGENTHORSY_CONFIG)
