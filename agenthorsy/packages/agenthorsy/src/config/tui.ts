...
import { isRecord } from "@agenthorsy-ai/tui/util/record"
...
import { TuiKeybind } from "@agenthorsy-ai/tui/config/keybind"
...
import { TuiConfig } from "@agenthorsy-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@agenthorsy/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old agenthorsy.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.agenthorsy`
  // folders between cwd and home, and AGENTHORSY_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.AGENTHORSY_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit AGENTHORSY_TUI_CONFIG override, if set.
  if (Flag.AGENTHORSY_TUI_CONFIG) {
    const configFile = Flag.AGENTHORSY_TUI_CONFIG
  ...
  // 4. `.agenthorsy` directories (and AGENTHORSY_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".agenthorsy") || dir === Flag.AGENTHORSY_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".agenthorsy") && dir !== Flag.AGENTHORSY_CONFIG_DIR) continue
