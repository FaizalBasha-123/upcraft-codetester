...
import { SkillPlugin } from "@agenthorsy-ai/core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@agenthorsy-ai/core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@agenthorsy-ai/core/v1/config/error"
...
const AGENTHORSY_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with agenthorsy. The model's intuition for what an
// agenthorsy.json should look like is often wrong, and agenthorsy hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch agenthorsy's own config files gives it the
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, AGENTHORSY_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@agenthorsy/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_AGENTHORSY_SKILL_NAME] = {
          name: CUSTOMIZE_AGENTHORSY_SKILL_NAME,
          description: CUSTOMIZE_AGENTHORSY_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_AGENTHORSY_SKILL_BODY,
        }
