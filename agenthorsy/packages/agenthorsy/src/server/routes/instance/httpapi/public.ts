...
import { AgentHorsyHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for agenthorsy serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AgentHorsyHttpApi)
...
export const PublicApi = AgentHorsyHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "agenthorsy",
    version: "1.0.0",
    description: "agenthorsy api",
