...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@agenthorsy-ai/sdk/v2"
...
export class Service extends Context.Service<Service, Interface>()("@agenthorsy/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `agenthorsy auth login` in the terminal",
      name: "Login with agenthorsy",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, agenthorsy provider, then sorted best model keep
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "AgentHorsy prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "AgentHorsy service failure", service })
}
