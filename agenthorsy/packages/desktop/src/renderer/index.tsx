...
import {
  ...
  useCommand,
  useWslServers,
} from "@agenthorsy-ai/app"
import type { UpdaterState } from "@agenthorsy-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@agenthorsy-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.AGENTHORSY_CHANNEL === "prod" &&
...
const deepLinkEvent = "agenthorsy:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__AGENTHORSY__ ??= {}
  const pending = window.__AGENTHORSY__.deepLinks ?? []
  window.__AGENTHORSY__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://agenthorsy.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("agenthorsy.global.dat").getItem("language")
