...
import { SessionMessageTable } from "@agenthorsy-ai/core/session/sql"
...
import { LocationServiceMap, locationServiceMapLayer } from "@agenthorsy-ai/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://agenthorsy.ai/config.json", ...config }),
  )
})
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/agenthorsy/src/session/processor.ts")
