...
import { EventSequenceTable } from "@agenthorsy-ai/core/event/sql"
...
import { WorkspaceV2 } from "@agenthorsy-ai/core/workspace"
import { WorkspaceTable } from "@agenthorsy-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  AGENTHORSY_AUTH_CONTENT: process.env.AGENTHORSY_AUTH_CONTENT,
  AGENTHORSY_EXPERIMENTAL_WORKSPACES: process.env.AGENTHORSY_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.AGENTHORSY_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.AGENTHORSY_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.AGENTHORSY_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.AGENTHORSY_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.AGENTHORSY_EXPERIMENTAL_WORKSPACES).toBe("true")
