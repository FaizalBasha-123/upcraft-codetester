...
- Author package entrypoint is `@agenthorsy-ai/plugin/tui`.
...
```json
{
  "$schema": "https://agenthorsy.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@agenthorsy-ai/plugin/tui`.
- `@agenthorsy-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@agenthorsy-ai/plugin/tui"
...
- The value is a semver range checked against the running AgentHorsy version.
...
When `plugin` entries exist in a writable `.opencode` dir or `AGENTHORSY_CONFIG_DIR`, AgentHorsy installs `@agenthorsy-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@agenthorsy-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus AgentHorsy's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
AgentHorsy registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, AgentHorsy pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@agenthorsy-ai/plugin/tui`.
...
- `--pure` / `AGENTHORSY_PURE` skips external TUI plugins only.
