...
- The server still lives in `packages/agenthorsy`.
...
Until enough shared service code lives outside `packages/agenthorsy`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/agenthorsy`

It should not import `packages/agenthorsy` services while `packages/agenthorsy`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/agenthorsy` implementation details.
