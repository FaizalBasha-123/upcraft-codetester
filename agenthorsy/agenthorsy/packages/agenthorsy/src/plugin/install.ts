...
export type PatchDeps = {
  ...
  files: (dir: string, name: "agenthorsy" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".agenthorsy")
}

function patchName(kind: Kind): "agenthorsy" | "tui" {
  if (kind === "server") return "agenthorsy"
  return "tui"
}
