...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@agenthorsy-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@agenthorsy-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@agenthorsy-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@agenthorsy-ai/tui/context/theme"
import { Global } from "@agenthorsy-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@agenthorsy-ai/core/util/flock"
import { Flag } from "@agenthorsy-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@agenthorsy-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@agenthorsy-ai/core/v1/config/plugin"
import { createCommandShim } from "@agenthorsy-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@agenthorsy-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".agenthorsy"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".agenthorsy", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".agenthorsy", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.AGENTHORSY_PURE ? [] : pluginOrigins
    if (Flag.AGENTHORSY_PURE && pluginOrigins.length) {
    }
