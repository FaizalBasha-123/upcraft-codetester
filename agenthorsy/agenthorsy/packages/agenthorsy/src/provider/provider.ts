...
import { Hash } from "@agenthorsy-ai/core/util/hash"
...
import { optional } from "@agenthorsy-ai/core/schema"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@agenthorsy-ai/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
  ...
  return {
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from agenthorsy.json provider config
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://agenthorsy.ai/",
            "X-Title": "opencode",
            "X-Source": "opencode",
          },
        },
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://agenthorsy.ai/",
            "X-Title": "opencode",
            "X-BILLING-INVOKE-ORIGIN": "AgentHorsy",
          },
        },
      }),
    vercel: () =>
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `agenthorsy auth cloudflare-ai-gateway`.",
        )
      }
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, agenthorsy auth, or provider options.`,
            )
          },
        }
      }
...
export class Service extends Context.Service<Service, Interface>()("@agenthorsy/Provider") {}
