...
```json
  "plugin": ["../packages/agenthorsy/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate AgentHorsy server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/agenthorsy/script/run-workspace-server
```

With the plugin install, you can now run AgentHorsy and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
