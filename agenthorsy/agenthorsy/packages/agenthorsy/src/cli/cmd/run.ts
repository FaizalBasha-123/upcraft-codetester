import type { PermissionV1 } from "@agenthorsy-ai/core/v1/permission"
import { FSUtil } from "@agenthorsy-ai/core/fs-util"
// CLI entry point for `agenthorsy run` and `agenthorsy --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`agenthorsy --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`agenthorsy --mini --attach`): connects to a running
//      agenthorsy server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@agenthorsy-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run agenthorsy with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running agenthorsy server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to AGENTHORSY_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to AGENTHORSY_SERVER_USERNAME or 'agenthorsy')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const sdk = createOpencodeClient({
        baseUrl: "http://agenthorsy.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
