...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall agenthorsy and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall AgentHorsy")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g agenthorsy-ai",
      pnpm: "pnpm uninstall -g agenthorsy-ai",
      bun: "bun remove -g agenthorsy-ai",
      yarn: "yarn global remove agenthorsy-ai",
      brew: "brew uninstall agenthorsy",
      choco: "choco uninstall agenthorsy",
      scoop: "scoop uninstall agenthorsy",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "agenthorsy", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using AgentHorsy!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# agenthorsy") || content.includes(".agenthorsy/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".agenthorsy/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }
    ...
    if (
      ...
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".agenthorsy"))
    ) {
      continue
    }
