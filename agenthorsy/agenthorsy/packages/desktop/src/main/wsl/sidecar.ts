...
import { type WslCommandLine, resolveWslAgenthorsy, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const opencode = await resolveWslAgenthorsy(distro)
  if (!opencode) throw new Error(`AgentHorsy is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "agenthorsy"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export AGENTHORSY_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export AGENTHORSY_CLIENT=desktop",
    `export AGENTHORSY_SERVER_USERNAME=${shellEscape(username)}`,
    `export AGENTHORSY_SERVER_PASSWORD=${shellEscape(password)}`,
