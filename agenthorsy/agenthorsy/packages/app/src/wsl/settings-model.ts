...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-agenthorsy" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslAgenthorsyAction(check?: WslAgenthorsyCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install Horsy"
  if (check.matchesDesktop === false) return "Update Horsy"
}
...
export function addServerViewModel(input: {
  ...
  const agenthorsyCheck = selectedDistro ? (state?.agenthorsyChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.agenthorsyChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateHorsy" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.agenthorsyMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installAgenthorsy" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  agenthorsyCheck: WslAgenthorsyCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingOpencode =
    input.state?.job?.kind === "install-agenthorsy" && input.state.job.distro === input.selectedDistro
  ...
  if (!addServerAgenthorsyReady(input.agenthorsyCheck)) {
    const update = !!input.agenthorsyCheck?.resolvedPath && input.agenthorsyCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingOpencode
        ? { key: "wsl.onboarding.updatingHorsy" }
        : update
          ? { key: "wsl.onboarding.updateHorsy" }
          : { key: "wsl.onboarding.installAgenthorsy" },
      disabled: !!input.state?.job || input.adding,
      action: "install-agenthorsy",
...
function addServerAgenthorsyReady(check: WslAgenthorsyCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.agenthorsyChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.agenthorsyChecks[item.name]) return [`agenthorsy:${item.name}`]
    return []
  })
