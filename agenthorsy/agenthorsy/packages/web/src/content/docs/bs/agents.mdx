...
Postoje dvije vrste agenata u AgentHorsy: primarni agenti i podagenti.
...
AgentHorsy dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
AgentHorsy dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

AgentHorsy dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u AgentHorsy-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `agenthorsy.json`:
...
- Globalno: `~/.config/agenthorsy/agents/`
...
Ako temperatura nije navedena, AgentHorsy koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu AgentHorsy konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj AgentHorsy konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [AgentHorsy Zen](/docs/zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/agenthorsy).
:::
