---
title: Konfiguracija
description: Korištenje AgentHorsy JSON konfiguracije.
---

Možete konfigurirati AgentHorsy koristeći JSON konfiguracijski fajl.

---

## Format

AgentHorsy podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/agenthorsy`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/agenthorsy/agenthorsy.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`agenthorsy.json` u projektu) - postavke specifične za projekat
...
:::note
Direktoriji `.opencode` i `~/.config/agenthorsy` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/agenthorsy` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu AgentHorsy konfiguraciju u `~/.config/agenthorsy/agenthorsy.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/agenthorsy/tui.json`.
...
Dodajte `agenthorsy.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se AgentHorsy pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json).

TUI konfiguracija koristi [**`agenthorsy.ai/tui.json`**](https://agenthorsy.ai/tui.json).
...
Stari `theme`, `keybinds`, i `tui` ključevi u `agenthorsy.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš AgentHorsy server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj AgentHorsy konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, AgentHorsy pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/agenthorsy/agents/` ili `.opencode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, AgentHorsy će se vratiti na `"build"` s upozorenjem.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/agenthorsy/commands/` ili `.opencode/commands/`. [Saznajte više ovdje](/docs/commands).
...
AgentHorsy će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, AgentHorsy **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju AgentHorsy sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.opencode/plugins/` ili `~/.config/agenthorsy/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite AgentHorsy da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
