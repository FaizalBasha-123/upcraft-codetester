...
Agent skills omogucavaju AgentHorsy da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
AgentHorsy pretrazuje ove lokacije:

- Konfiguracija projekta: `.opencode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/agenthorsy/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, AgentHorsy ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
...
Globalne definicije se takoder ucitavaju iz `~/.config/agenthorsy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
AgentHorsy navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `agenthorsy.json`:
...
**Za ugradene agente** (u `agenthorsy.json`):
