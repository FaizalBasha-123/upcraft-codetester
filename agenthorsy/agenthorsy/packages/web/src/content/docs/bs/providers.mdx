---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u AgentHorsy.
---

import config from "../../../../config.mjs"
export const console = config.console

AgentHorsy koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj AgentHorsy konfiguraciji.
...
u `~/.local/share/agenthorsy/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem AgentHorsy
config.
...
AgentHorsy Zen je lista modela koje je obezbedio AgentHorsy tim koji su bili
testirano i potvrđeno da dobro radi sa AgentHorsy. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa AgentHorsy Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite opencode i idite na [agenthorsy.ai/auth](https://agenthorsy.ai/auth).
...
Radi kao i svaki drugi provajder u AgentHorsy i potpuno je opcionalan za korištenje.

---

## AgentHorsy Go

AgentHorsy Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža AgentHorsy tim i koji su testirani i verificirani da dobro rade s AgentHorsy-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `AgentHorsy Go` i idite na [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
Da biste koristili Amazon Bedrock s AgentHorsy:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `agenthorsy.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u AgentHorsy.
:::
...
AgentHorsy koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate AgentHorsy
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`agenthorsy.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth).

##### Konfiguracija

Prilagodite putem `agenthorsy.json`:
...
Za korištenje Google Vertex AI s AgentHorsy:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) da automatski prijavite svaki AgentHorsy razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki AgentHorsy razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za AgentHorsy. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/agenthorsy) za detalje.
:::
...
Da biste koristili Ollama Cloud s AgentHorsy:
...
3. Kopirajte API ključ za korištenje u AgentHorsy.
...
6. **Važno**: Prije upotrebe modela oblaka u AgentHorsy, morate lokalno povući informacije o modelu:
...
AgentHorsy Zen je lista testiranih i verifikovanih modela koju je obezbedio AgentHorsy tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>AgentHorsy Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **AgentHorsy Zen**.

```txt
   /connect
```

3. Unesite svoj AgentHorsy API ključ.
...
4. Kreirajte ili ažurirajte svoju `agenthorsy.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju AgentHorsy da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite AgentHorsy konfiguraciju i:
