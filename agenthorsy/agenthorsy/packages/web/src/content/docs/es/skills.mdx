...
Las habilidades del agente permiten a AgentHorsy descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
AgentHorsy busca estas ubicaciones:

- Configuración del proyecto: `.opencode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/agenthorsy/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, AgentHorsy sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
...
Las definiciones globales también se cargan desde `~/.config/agenthorsy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
AgentHorsy enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `agenthorsy.json`:
...
**Para agentes integrados** (en `agenthorsy.json`):
