...
-**macOS/리눅스**: `~/.local/share/agenthorsy/log/`
...
-**macOS/리눅스**: `~/.local/share/agenthorsy/`
...
- macOS만: `AgentHorsy` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
-**macOS/리눅스**: `~/.config/agenthorsy/agenthorsy.jsonc` (또는 `~/.config/agenthorsy/agenthorsy.json`) -**macOS/Linux** (외부 설치): `~/.local/share/agenthorsy/agenthorsy.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
- **글로벌 플러그인** -**macOS/리눅스**: `~/.config/agenthorsy/plugins/`
...
-**macOS**: 찾기 -> `Cmd+Shift+G` -> 붙여넣기 `~/.cache/agenthorsy`

- **Linux**: `~/.cache/agenthorsy` 삭제 (또는 `rm -rf ~/.cache/agenthorsy` 실행)
...
`agenthorsy.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.
