---
title: 플러그인
description: AgentHorsy를 확장하기 위해 자신만의 플러그인을 작성하세요.
---
...
- `~/.config/agenthorsy/plugins/` - 글로벌 플러그인
...
**npm 플러그인**은 시작시 Bun을 사용하여 자동으로 설치됩니다. 패키지와 그 의존성은 `~/.cache/agenthorsy/node_modules/`에 캐시됩니다.
...
1. 글로벌 구성 (`~/.config/agenthorsy/agenthorsy.json`)
2. 프로젝트 구성 (`agenthorsy.json`)
3. 글로벌 플러그인 디렉토리 (`~/.config/agenthorsy/plugins/`)
...
레벨: `debug`, `info`, `warn`, `error`. [SDK 문서](https://agenthorsy.ai/docs/sdk)를 참고하세요.
