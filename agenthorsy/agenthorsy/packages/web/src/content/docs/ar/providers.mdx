---
title: المزوّدون
description: استخدام أي موفّر LLM في AgentHorsy.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم AgentHorsy ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات AgentHorsy.
...
في `~/.local/share/agenthorsy/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات AgentHorsy.
...
AgentHorsy Zen هي قائمة نماذج يوفّرها فريق AgentHorsy وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع AgentHorsy. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع AgentHorsy Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [agenthorsy.ai/auth](https://agenthorsy.ai/auth).
...
يعمل مثل أي مزوّد آخر في AgentHorsy واستخدامه اختياري بالكامل.

---

## AgentHorsy Go

AgentHorsy Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق AgentHorsy، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع AgentHorsy.

1. شغّل الأمر `/connect` في TUI، واختر `AgentHorsy Go`، ثم انتقل إلى [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
لاستخدام Amazon Bedrock مع AgentHorsy:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `agenthorsy.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل AgentHorsy غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم AgentHorsy نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر AgentHorsy
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`agenthorsy.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth).

##### التهيئة

خصّص عبر `agenthorsy.json`:
...
لاستخدام Google Vertex AI مع AgentHorsy:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) لتسجيل كل محادثة AgentHorsy تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة AgentHorsy مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع AgentHorsy. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/agenthorsy) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع AgentHorsy:
...
3. انسخ مفتاح API لاستخدامه في AgentHorsy.
...
6. **مهم**: قبل استخدام نماذج السحابة في AgentHorsy، يجب سحب معلومات النموذج محليا:
...
AgentHorsy Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق AgentHorsy. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>AgentHorsy Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **AgentHorsy Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ AgentHorsy.
...
4. أنشئ أو حدّث ملف `agenthorsy.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ AgentHorsy بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
