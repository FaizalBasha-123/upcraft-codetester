---
title: CLI
description: أوامر وخيارات CLI في AgentHorsy.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ AgentHorsy افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
opencode
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع AgentHorsy برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ AgentHorsy.
...
يتضمن CLI الخاص بـ AgentHorsy أيضا الأوامر التالية.

---

### agent

إدارة وكلاء AgentHorsy.
...
إرفاق terminal بخادم الواجهة الخلفية لـ AgentHorsy قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
opencode attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ AgentHorsy تعمل عن بعد. على سبيل المثال:
...
يعتمد AgentHorsy على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `opencode auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/agenthorsy/auth.json`.

```bash
opencode auth login
```

عند تشغيل AgentHorsy يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في AgentHorsy.
...
بدء خادم AgentHorsy بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات AgentHorsy لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة AgentHorsy.

```bash
opencode import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة AgentHorsy.
...
بدء خادم AgentHorsy بدون واجهة مع واجهة ويب.

```bash
opencode web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى AgentHorsy عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
إلغاء تثبيت AgentHorsy وإزالة جميع الملفات ذات الصلة.
...
يمكن تهيئة AgentHorsy باستخدام متغيرات البيئة.
