---
title: الويب
description: استخدام AgentHorsy في متصفحك.
---

يمكن تشغيل AgentHorsy كتطبيق ويب داخل متصفحك، ليمنحك تجربة البرمجة القوية بالذكاء الاصطناعي نفسها دون الحاجة إلى terminal.

![AgentHorsy Web - جلسة جديدة](../../../assets/web/web-homepage-new-session.png)
...
يؤدي ذلك إلى تشغيل خادم محلي على `127.0.0.1` بمنفذ عشوائي متاح، ويفتح AgentHorsy تلقائيا في المتصفح الافتراضي لديك.
...
افتراضيا، يختار AgentHorsy منفذا متاحا. يمكنك تحديد منفذ:
...
افتراضيا، يرتبط الخادم بـ `127.0.0.1` (للاستخدام المحلي فقط). لجعل AgentHorsy متاحا على شبكتك:

```bash
opencode web --hostname 0.0.0.0
```

عند استخدام `0.0.0.0` سيعرض AgentHorsy كلا من عناوين الوصول المحلي وعناوين الشبكة:
...
بعد التشغيل، تتيح لك واجهة الويب الوصول إلى جلسات AgentHorsy الخاصة بك.

### الجلسات

اعرض جلساتك وأدرها من الصفحة الرئيسية. يمكنك رؤية الجلسات النشطة وبدء جلسات جديدة.

![AgentHorsy Web - جلسة نشطة](../../../assets/web/web-homepage-active-session.png)

### حالة الخادم

انقر على "See Servers" لعرض الخوادم المتصلة وحالتها.

![AgentHorsy Web - See Servers (عرض الخوادم)](../../../assets/web/web-homepage-see-servers.png)
...
يمكنك أيضا ضبط إعدادات الخادم داخل ملف الإعدادات `agenthorsy.json`:
