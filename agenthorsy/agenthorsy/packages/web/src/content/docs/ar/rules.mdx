...
يمكنك أيضًا وضع قواعد عامة في ملف `~/.config/agenthorsy/AGENTS.md`. تُطبَّق هذه القواعد على جميع جلسات opencode.
...
للمستخدمين الذين ينتقلون من Claude Code، يدعم AgentHorsy اصطلاحات ملفات Claude Code كبدائل احتياطية:

- **قواعد المشروع**: `CLAUDE.md` في دليل مشروعك (يُستخدم إذا لم يوجد `AGENTS.md`)
- **قواعد عامة**: `~/.claude/CLAUDE.md` (يُستخدم إذا لم يوجد `~/.config/agenthorsy/AGENTS.md`)
...
2. **ملف عام** في `~/.config/agenthorsy/AGENTS.md`
3. **ملف Claude Code** في `~/.claude/CLAUDE.md` (ما لم يتم تعطيله)

يتم اعتماد أول ملف مطابق ضمن كل فئة. على سبيل المثال، إذا كان لديك كل من `AGENTS.md` و`CLAUDE.md` فسيتم استخدام `AGENTS.md` فقط. وبالمثل، تكون الأولوية لملف `~/.config/agenthorsy/AGENTS.md` على `~/.claude/CLAUDE.md`.

---

## تعليمات مخصصة

يمكنك تحديد ملفات تعليمات مخصصة في `agenthorsy.json` أو في الملف العام `~/.config/agenthorsy/agenthorsy.json`. يتيح ذلك لك ولفريقك إعادة استخدام القواعد الموجودة بدلًا من تكرارها داخل AGENTS.md.
...
### استخدام agenthorsy.json

النهج الموصى به هو استخدام الحقل `instructions` في `agenthorsy.json`:
...
:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `agenthorsy.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::
