...
- Tylko macOS: menu `AgentHorsy` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/agenthorsy/agenthorsy.jsonc` (lub `~/.config/agenthorsy/agenthorsy.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
- **Linux**: usuń `~/.cache/agenthorsy` (lub uruchom `rm -rf ~/.cache/agenthorsy`)
...
Jeśli Twój `agenthorsy.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
