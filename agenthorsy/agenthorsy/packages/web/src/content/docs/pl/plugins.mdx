...
- `~/.config/agenthorsy/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/agenthorsy/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/agenthorsy/agenthorsy.json`)
2. Konfiguracja projektu (`agenthorsy.json`)
3. Globalny katalog wtyczek (`~/.config/agenthorsy/plugins/`)
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://agenthorsy.ai/docs/sdk).
