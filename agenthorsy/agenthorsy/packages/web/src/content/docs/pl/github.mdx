...
opencode integruje się z przepływem pracy w GitHub. Wspomnij o `/agenthorsy` lub `/oc` w swoim komentarzu, a opencode wykonaj zadania w ramach modułu uruchamiającego GitHub Actions.
...
1. **Zainstaluj aplikację GitHub**
   ...
   Wejdź na [**github.com/apps/agenthorsy-agent**](https://github.com/apps/agenthorsy-agent). wystąpienie się, że jest natychmiastowe w repozytorium usuwam.

2. **Dodaj przepływ pracy**

   Dodaj zdalny plik pracy do `.github/workflows/agenthorsy.yml` w swoim repozytorium. wystąpił, że ustawiłeś sędziego `model` i wymagany klucz API w `env`.
...
| ----------------------------- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Skomentuj problem lub PR                         | Wspomnij o `/agenthorsy` lub `/oc` w swoim komentarzu. opencode odczytuje kontekst i może być częścią składową, otwieraną przez PR lub odpowiedzialną.  |
| `pull_request_review_comment` | Komentarz określonym kodem w PR                  | Wspomnij o `/agenthorsy` lub `/oc` podczas przeglądania kodu. opencode źródło pochodzenia, numery linii i kontekst różnicowy.                           |
