...
w `~/.local/share/agenthorsy/auth.json`.
...
AgentHorsy Zen to lista modeli dostarczonych przez zespół opencode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opencode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od AgentHorsy Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `AgentHorsy Zen` i przejdź do [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
AgentHorsy Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół opencode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `AgentHorsy Go` i przejdź do [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `agenthorsy.json`:
...
`agenthorsy.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth).
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session), aby automatycznie rejestrować każdą konwersację opencode jako sesję w Helicone.
...
:::tip
Ollama może automatycznie skonfigurować się pod kątem opencode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/agenthorsy).
:::
...
AgentHorsy Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>AgentHorsy Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **AgentHorsy Zen**.
...
4. Utwórz lub zaktualizuj plik `agenthorsy.json` w katalogu projektu:
