---
title: Provider
description: Usa qualsiasi provider LLM in AgentHorsy.
---

import config from "../../../../config.mjs"
export const console = config.console

AgentHorsy usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di AgentHorsy.
...
AgentHorsy Zen e una lista di modelli forniti dal team AgentHorsy, testati e
verificati per funzionare bene con AgentHorsy. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da AgentHorsy Zen.
:::

1. Esegui `/connect` nella TUI, seleziona opencode e vai su [agenthorsy.ai/auth](https://agenthorsy.ai/auth).
...
Funziona come qualsiasi altro provider in AgentHorsy ed e totalmente opzionale.
...
Per usare Amazon Bedrock con AgentHorsy:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `agenthorsy.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in AgentHorsy non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
AgentHorsy usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare AgentHorsy
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `agenthorsy.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth).

##### Configurazione

Personalizza tramite `agenthorsy.json`:
...
Per usare Google Vertex AI con AgentHorsy:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) per loggare automaticamente ogni conversazione di AgentHorsy come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di AgentHorsy elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per AgentHorsy. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/agenthorsy) per dettagli.
:::
...
Per usare Ollama Cloud con AgentHorsy:
...
3. Copia la chiave API da usare in AgentHorsy.
...
6. **Importante**: Prima di usare modelli cloud in AgentHorsy, devi scaricare le informazioni del modello localmente:
...
AgentHorsy Zen è una lista di modelli testati e verificati forniti dal team AgentHorsy. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>AgentHorsy Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **AgentHorsy Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di AgentHorsy.
...
4. Crea o aggiorna il tuo file `agenthorsy.json` nella directory del progetto:
...
I campi `limit` permettono a AgentHorsy di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
