---
title: Plugin
description: Scrivi plugin per estendere AgentHorsy.
---

I plugin ti permettono di estendere AgentHorsy agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di AgentHorsy.
...
- `~/.config/agenthorsy/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/agenthorsy/node_modules/`.
...
1. Config globale (`~/.config/agenthorsy/agenthorsy.json`)
2. Config di progetto (`agenthorsy.json`)
3. Directory plugin globale (`~/.config/agenthorsy/plugins/`)
...
AgentHorsy esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
:::note
Se usi l'app desktop di AgentHorsy, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
I tuoi strumenti personalizzati saranno disponibili in agenthorsy insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://agenthorsy.ai/docs/sdk) per i dettagli.
