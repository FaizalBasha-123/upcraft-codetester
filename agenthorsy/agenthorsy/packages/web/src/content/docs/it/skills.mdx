...
Le skill degli agenti permettono a AgentHorsy di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
AgentHorsy cerca in queste posizioni:

- Config di progetto: `.opencode/skills/<name>/SKILL.md`
- Config globale: `~/.config/agenthorsy/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, AgentHorsy risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
...
Le definizioni globali vengono caricate anche da `~/.config/agenthorsy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
AgentHorsy elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `agenthorsy.json`:
...
**Per agenti integrati** (in `agenthorsy.json`):
