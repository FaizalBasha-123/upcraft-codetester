...
Puoi aggiungere strumenti esterni a AgentHorsy usando il _Model Context Protocol_, o MCP. AgentHorsy supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [AgentHorsy Config](https://agenthorsy.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/agenthorsy`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
AgentHorsy gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, AgentHorsy:
...
Se il server richiede autenticazione, AgentHorsy ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `opencode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, AgentHorsy memorizzera' i token in modo sicuro in `~/.local/share/agenthorsy/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in AgentHorsy insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione AgentHorsy come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare AgentHorsy al tuo account Sentry.
