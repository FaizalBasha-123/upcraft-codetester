---
title: Plugins
description: Write your own plugins to extend AgentHorsy.
---

Plugins allow you to extend AgentHorsy by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify AgentHorsy's default behavior.
...
- `~/.config/agenthorsy/plugins/` - Global plugins
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/agenthorsy/node_modules/`.
...
3. Global plugin directory (`~/.config/agenthorsy/plugins/`)
...
AgentHorsy runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the AgentHorsy desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://agenthorsy.ai/docs/sdk) for details.
