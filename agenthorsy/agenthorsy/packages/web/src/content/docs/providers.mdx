---
title: Providers
description: Using any LLM provider in AgentHorsy.
---

import config from "../../../config.mjs"
export const console = config.console

AgentHorsy uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your AgentHorsy config.
...
in `~/.local/share/agenthorsy/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your AgentHorsy
config.
...
AgentHorsy Zen is a list of models provided by the AgentHorsy team that have been
tested and verified to work well with AgentHorsy. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with AgentHorsy Zen.
:::

1. Run the `/connect` command in the TUI, select `AgentHorsy Zen`, and head to [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
It works like any other provider in AgentHorsy and is completely optional to use.

---

## AgentHorsy Go

AgentHorsy Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the AgentHorsy team that have been
tested and verified to work well with AgentHorsy.

1. Run the `/connect` command in the TUI, select `AgentHorsy Go`, and head to [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
To use Amazon Bedrock with AgentHorsy:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `agenthorsy.json`:

   ```json title="agenthorsy.json"
   {
     "$schema": "https://agenthorsy.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
AgentHorsy. Anthropic explicitly prohibits this.

Previous versions of AgentHorsy came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in AgentHorsy with zero setup:
...
```json title="agenthorsy.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for agenthorsy to work properly.
   :::
...
AgentHorsy supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; AgentHorsy uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize AgentHorsy. Sign in and approve.

   :::note
   AgentHorsy requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, AgentHorsy surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — AgentHorsy forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in AgentHorsy is experimental. Features, configuration, and
behavior may change in future releases.
:::

AgentHorsy integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in agenthorsy auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
AgentHorsy uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock AgentHorsy
to only use your own GitLab-hosted instance, add the following to your
`agenthorsy.json` file. It is also recommended to disable session sharing.
...
More documentation on [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth) homepage.

##### Configuration

Customize through `agenthorsy.json`:
...
When a `duo-workflow-*` model is selected, AgentHorsy will:
...
4. Route tool execution requests through AgentHorsy's permission-gated tool system
...
To use your GitHub Copilot subscription with agenthorsy:
...
To use GMI Cloud with AgentHorsy:
...
To use Google Vertex AI with AgentHorsy:
...
```jsonc title="~/.config/agenthorsy/agenthorsy.jsonc"
{
  "$schema": "https://agenthorsy.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) plugin to automatically log each AgentHorsy conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each AgentHorsy conversation listed as a separate session.
...
```json title="agenthorsy.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="agenthorsy.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "provider": {
    "lmstudio": {
...
:::tip
Ollama can automatically configure itself for AgentHorsy. See the [Ollama integration docs](https://docs.ollama.com/integrations/agenthorsy) for details.
:::

```json title="agenthorsy.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with AgentHorsy:
...
3. Copy the API key for use in AgentHorsy.
...
6. **Important**: Before using cloud models in AgentHorsy, you must pull the model information locally:
...
AgentHorsy Zen is a list of tested and verified models provided by the AgentHorsy team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>AgentHorsy Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **AgentHorsy Zen**.

   ```txt
   /connect
   ```

3. Enter your AgentHorsy API key.
...
5. Complete sign-in in the browser that opens. AgentHorsy captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
The model catalog is provided automatically. A minimal `agenthorsy.json` is all that's needed:
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. AgentHorsy opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
AgentHorsy refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when AgentHorsy is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running AgentHorsy — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. AgentHorsy prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. AgentHorsy polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with agenthorsy. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in agenthorsy.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `agenthorsy.json` file in your project directory:

   ```json title="agenthorsy.json" ""myprovider"" {5-15}
   {
     "$schema": "https://agenthorsy.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="agenthorsy.json" {9,11,17-20}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow AgentHorsy to understand how much context you have left. Standard providers pull these from models.dev automatically.
