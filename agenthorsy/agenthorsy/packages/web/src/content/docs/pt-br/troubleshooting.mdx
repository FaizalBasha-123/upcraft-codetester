...
- Apenas macOS: menu `AgentHorsy` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/agenthorsy/agenthorsy.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\agenthorsy.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/agenthorsy`
- **Linux**: exclua `~/.cache/agenthorsy` (ou execute `rm -rf ~/.cache/agenthorsy`)
...
Se seu `agenthorsy.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
