...
1. **Configuração remota** (de `.well-known/agenthorsy`) - padrões organizacionais
2. **Configuração global** (`~/.config/agenthorsy/agenthorsy.json`) - preferências do usuário
3. **Configuração personalizada** (`OPENCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`agenthorsy.json` no projeto) - configurações específicas do projeto
...
:::note
Os diretórios `.opencode` e `~/.config/agenthorsy` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/agenthorsy`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do opencode em `~/.config/agenthorsy/agenthorsy.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/agenthorsy/tui.json`.
...
Adicione `agenthorsy.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json).

A configuração do TUI usa [**`agenthorsy.ai/tui.json`**](https://agenthorsy.ai/tui.json).
...
Chaves legadas `theme`, `keybinds` e `tui` em `agenthorsy.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/agenthorsy/agents/` ou `.opencode/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/agenthorsy/commands/` ou `.opencode/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.opencode/plugins/` ou `~/.config/agenthorsy/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
