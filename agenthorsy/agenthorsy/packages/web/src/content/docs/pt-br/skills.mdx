...
- Configuração global: `~/.config/agenthorsy/skills/<name>/SKILL.md`
...
As definições globais também são carregadas de `~/.config/agenthorsy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `agenthorsy.json`:
...
**Para agentes embutidos** (em `agenthorsy.json`):
