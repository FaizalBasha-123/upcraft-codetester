---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i AgentHorsy.
---

import config from "../../../../config.mjs"
export const console = config.console

AgentHorsy bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i AgentHorsy-konfigurasjonen.
...
i `~/.local/share/agenthorsy/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i AgentHorsy
konfig.
...
AgentHorsy Zen er en liste over modeller levert av AgentHorsy-teamet som har vært
testet og verifisert for å fungere godt med AgentHorsy. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med AgentHorsy Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `AgentHorsy Zen` og gå til [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
Det fungerer som alle andre leverandører i AgentHorsy og er helt valgfritt å bruke.

---

## AgentHorsy Go

AgentHorsy Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av AgentHorsy-teamet som har vært
testet og verifisert for å fungere godt med AgentHorsy.

1. Kjør kommandoen `/connect` i TUI, velg `AgentHorsy Go`, og gå til [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
Slik bruker du Amazon Bedrock med AgentHorsy:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører AgentHorsy:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `agenthorsy.json`:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at AgentHorsy skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom AgentHorsy-konfigurasjonen.
...
for å lagre token i AgentHorsy auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
AgentHorsy bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse AgentHorsy
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`agenthorsy.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `agenthorsy.json`:
...
Slik bruker du GitHub Copilot-abonnementet med AgentHorsy:
...
Slik bruker du Google Vertex AI med AgentHorsy:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører AgentHorsy.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom AgentHorsy, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session) plugin for automatisk å logge hver AgentHorsy-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver AgentHorsy-samtale oppført som en separat økt.
...
Du kan konfigurere AgentHorsy for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere AgentHorsy for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere AgentHorsy for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for AgentHorsy. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/agenthorsy) for detaljer.
:::
...
Slik bruker du Ollama Cloud med AgentHorsy:
...
3. Kopier API-nøkkelen for bruk i AgentHorsy.
...
6. **Viktig**: Før du bruker skymodeller i AgentHorsy, må du hente modellinformasjonen lokalt:
...
AgentHorsy Zen er en liste over testede og verifiserte modeller levert av AgentHorsy-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>AgentHorsy Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **AgentHorsy Zen**.

   ```txt
   /connect
   ```

3. Skriv inn AgentHorsy API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom AgentHorsy-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom AgentHorsy-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AgentHorsy:
...
Du kan også tilpasse modeller gjennom AgentHorsy-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med AgentHorsy. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `agenthorsy.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar AgentHorsy forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk AgentHorsy-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i AgentHorsy-konfigurasjonen.
