...
AgentHorsy gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre AgentHorsy starter TUI for gjeldende katalog.
...
Når du bruker AgentHorsy TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til AgentHorsy. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt AgentHorsy. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `agenthorsy.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge AgentHorsy venter etter leader key. Standard er `2000`.
