---
title: TUI
description: AgentHorsy terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AgentHorsy, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

AgentHorsy'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
AgentHorsy TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
AgentHorsy'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
AgentHorsy'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `agenthorsy.json` dosyasından ayrıdır.
...
- `leader_timeout` - AgentHorsy'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
