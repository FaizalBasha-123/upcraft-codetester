---
title: Konfiguration
description: Verwenden Sie die Konfiguration AgentHorsy JSON.
---

Sie können AgentHorsy mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

AgentHorsy unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
1. **Remote-Konfiguration** (von `.well-known/agenthorsy`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/agenthorsy/agenthorsy.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`OPENCODE_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`agenthorsy.json` im Projekt) – projektspezifische Einstellungen
...
:::note
Die Verzeichnisse `.opencode` und `~/.config/agenthorsy` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/agenthorsy` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale AgentHorsy-Konfiguration in `~/.config/agenthorsy/agenthorsy.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `agenthorsy.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn AgentHorsy gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Die Konfigurationsdatei verfügt über ein Schema, das in [**`agenthorsy.ai/config.json`**](https://agenthorsy.ai/config.json) definiert ist.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres AgentHorsy-Servers erkannt werden.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer AgentHorsy-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht AgentHorsy, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer AgentHorsy-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/agenthorsy/agents/` oder `.opencode/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift AgentHorsy mit einer Warnung auf `"build"` zurück.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/agenthorsy/commands/` oder `.opencode/commands/` definieren. [Mehr erfahren](/docs/commands).
...
AgentHorsy lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
AgentHorsy erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert AgentHorsy mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.opencode/plugins/` oder `~/.config/agenthorsy/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie AgentHorsy darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
