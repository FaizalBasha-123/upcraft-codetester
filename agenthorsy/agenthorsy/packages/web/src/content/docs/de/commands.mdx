...
Sie können benutzerdefinierte Befehle über die AgentHorsy-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem AgentHorsy [config](/docs/config):

```json title="agenthorsy.jsonc" {4-12}
{
...
- Global: `~/.config/agenthorsy/commands/`
- Pro Projekt: `.opencode/commands/`

```markdown title="~/.config/agenthorsy/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
AgentHorsy enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
