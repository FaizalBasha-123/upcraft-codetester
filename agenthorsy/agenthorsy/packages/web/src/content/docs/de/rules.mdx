...
Um eine neue `AGENTS.md` zu erstellen, fuehre in agenthorsy den Befehl `/init` aus.
...
Du kannst auch globale Regeln in `~/.config/agenthorsy/AGENTS.md` speichern.
...
Wenn du von Claude Code kommst, unterstuetzt AgentHorsy dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/agenthorsy/AGENTS.md` existiert)
...
2. **Globale Datei** unter `~/.config/agenthorsy/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `agenthorsy.json` oder in `~/.config/agenthorsy/agenthorsy.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `agenthorsy.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `agenthorsy.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
