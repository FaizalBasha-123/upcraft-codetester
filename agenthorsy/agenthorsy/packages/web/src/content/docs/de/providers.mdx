---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in AgentHorsy.
---

import config from "../../../../config.mjs"
export const console = config.console

AgentHorsy verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer AgentHorsy-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem AgentHorsy anpassen
config.
...
AgentHorsy Zen ist eine Liste von Modellen, die vom AgentHorsy-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit AgentHorsy funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit AgentHorsy Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie AgentHorsy aus und gehen Sie zu [agenthorsy.ai/auth](https://agenthorsy.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in AgentHorsy und ist völlig optional.

---

## AgentHorsy Go

AgentHorsy Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom AgentHorsy-Team getestet und verifiziert wurden, dass sie gut mit AgentHorsy funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `AgentHorsy Go` und gehen Sie zu [agenthorsy.ai/auth](https://agenthorsy.ai/zen).
...
So verwenden Sie Amazon Bedrock mit AgentHorsy:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie AgentHorsy ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `agenthorsy.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in AgentHorsy wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit AgentHorsy ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre AgentHorsy-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `AgentHorsy`, Bereiche: `api`
...
um das Token im AgentHorsy-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
AgentHorsy verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von AgentHorsy
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`agenthorsy.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [opencode-gitlab-auth](https://www.npmjs.com/package/agenthorsy-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `agenthorsy.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit AgentHorsy:
...
So verwenden Sie Google Vertex AI mit AgentHorsy:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie AgentHorsy ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über AgentHorsy konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/agenthorsy-helicone-session)-Plugin, um jede AgentHorsy-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede AgentHorsy-Konversation als separate Sitzung aufgeführt.
...
Sie können AgentHorsy für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können AgentHorsy über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können AgentHorsy über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für AgentHorsy konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/agenthorsy).
:::
...
So verwenden Sie Ollama Cloud mit AgentHorsy:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in AgentHorsy.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in AgentHorsy verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
AgentHorsy Zen ist eine Liste getesteter und verifizierter Modelle, die vom AgentHorsy-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>AgentHorsy Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **AgentHorsy Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren AgentHorsy API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre AgentHorsy-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre AgentHorsy-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit AgentHorsy:
...
Sie können Modelle auch über Ihre AgentHorsy-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit AgentHorsy verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `agenthorsy.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann AgentHorsy erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die AgentHorsy-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer AgentHorsy-Konfiguration übereinstimmt.
