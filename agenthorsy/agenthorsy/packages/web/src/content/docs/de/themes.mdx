...
Mit AgentHorsy kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet AgentHorsy das `opencode`-Theme.
...
AgentHorsy liefert mehrere eingebaute Themes mit.
...
- AgentHorsy optisch ans Terminal anpassen wollen
...
AgentHorsy unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/agenthorsy/themes/*.json` oder `$XDG_CONFIG_HOME/agenthorsy/themes/*.json`
