...
Agent Skills erlauben AgentHorsy, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
AgentHorsy sucht in folgenden Pfaden:

- Project config: `.opencode/skills/<name>/SKILL.md`
- Global config: `~/.config/agenthorsy/skills/<name>/SKILL.md`
...
Bei projektlokalen Pfaden laeuft AgentHorsy vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
...
Globale Definitionen kommen zusaetzlich aus `~/.config/agenthorsy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
AgentHorsy listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `agenthorsy.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `agenthorsy.json`):
