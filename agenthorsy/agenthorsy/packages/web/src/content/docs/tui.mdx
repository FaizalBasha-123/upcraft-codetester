---
title: TUI
description: Using the AgentHorsy terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AgentHorsy provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running AgentHorsy starts the TUI for the current directory.
...
When using the AgentHorsy TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to AgentHorsy. Allows you to select from available providers and add their API keys.
...
Exit AgentHorsy. _Aliases_: `/quit`, `/q`
...
This is separate from `agenthorsy.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long AgentHorsy waits after the leader key. Defaults to `2000`.
