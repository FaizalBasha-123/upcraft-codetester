---
title: CLI
description: AgentHorsy CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AgentHorsy CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med AgentHorsy programmatisk.
...
Start AgentHorsy terminalbrugergrænsefladen.
...
Administrer agenter i AgentHorsy.
...
Tilslut en terminal til en allerede kørende AgentHorsy backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern AgentHorsy backend. For eksempel:
...
AgentHorsy drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/agenthorsy/auth.json`.

```bash
opencode auth login
```

Når AgentHorsy starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i AgentHorsy.
...
Start en hovedløs AgentHorsy-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.
...
Administrator AgentHorsy sessionsholder.
...
Liste over alle AgentHorsy sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine AgentHorsy-sessioner.
...
Importer sessionsdata fra en JSON fil eller AgentHorsy del URL.
...
Start en hovedløs AgentHorsy-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til AgentHorsy via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller AgentHorsy og fjern alle relaterede filer.
...
Opdaterer AgentHorsy til den nyeste version eller en specifik version.
...
agenthorsy CLI tager følgende globale flag.
...
AgentHorsy kan konfigureres ved hjælp af miljøvariabler.
