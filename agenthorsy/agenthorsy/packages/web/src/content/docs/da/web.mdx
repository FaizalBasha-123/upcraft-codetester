---
title: Web
description: Brug af AgentHorsy i din browser.
---

AgentHorsy kan køre som en webapplikation i din browser, hvilket giver den samme kraftfulde AI-kodningsoplevelse uden behov for en terminal.

![AgentHorsy Web - Ny session](../../../assets/web/web-homepage-new-session.png)
...
Dette starter en lokal server på `127.0.0.1` med en tilfældig tilgængelig port og åbner automatisk AgentHorsy i din standardbrowser.
...
Som standard vælger AgentHorsy en tilgængelig port. Du kan angive en port:
...
Som standard binder serveren til `127.0.0.1` (kun lokal vært). Sådan gør du AgentHorsy tilgængelig på dit netværk:

```bash
opencode web --hostname 0.0.0.0
```

Når du bruger `0.0.0.0`, vil AgentHorsy vise både lokale adresser og netværksadresser:
...
Når den er startet, giver webgrænsefladen adgang til dine AgentHorsy-sessioner.

### Sessioner

Se og administrer dine sessioner fra hjemmesiden. Du kan se aktive sessioner og starte nye.

![AgentHorsy Web - Active Session](../../../assets/web/web-homepage-active-session.png)

### Serverstatus

Klik på "Se servere" for at se tilsluttede servere og deres status.

![AgentHorsy Web - Se servere](../../../assets/web/web-homepage-see-servers.png)
...
Du kan også konfigurere serverindstillinger i din `agenthorsy.json`-konfigurationsfil:
