...
- `~/.config/agenthorsy/plugins/` — глобальные плагины
...
**Плагины npm** устанавливаются автоматически с помощью Bun при запуске. Пакеты и их зависимости кэшируются в `~/.cache/agenthorsy/node_modules/`.
...
1. Глобальная конфигурация (`~/.config/agenthorsy/agenthorsy.json`)
2. Конфигурация проекта (`agenthorsy.json`)
3. Глобальный каталог плагинов (`~/.config/agenthorsy/plugins/`)
...
Уровни: `debug`, `info`, `warn`, `error`. Подробности см. в документации SDK](https://agenthorsy.ai/docs/sdk).
