...
There are two types of agents in AgentHorsy; primary agents and subagents.
...
AgentHorsy comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
AgentHorsy comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

AgentHorsy comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into AgentHorsy's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
An autonomous agent that executes complex tasks end-to-end using plan-then-build methodology. It is the primary agent used by the orchestrator for task execution.

**How it works:**

1. **Plan mode** — Reads the task specification, explores the codebase, and creates a structured plan at `plans/<task-name>.md` with acceptance criteria and verification steps.
2. **Build mode** — Executes the plan's acceptance criteria as todos, making file changes, running tests, and updating context files (`architecture/`, `database_metadata/`) as needed.
3. **Validation** — Before completing, runs `validate_task` to verify the implementation matches the plan, then `merge_worktree` to merge changes back.

**Permissions:** Full access to edit, plan files, architecture docs, database metadata, and all tools including `validate_task` and `merge_worktree`.

**When to use:** Automatically invoked by the orchestrator. Can also be invoked directly via `@dynamic_persona` for complex tasks that benefit from structured planning.

---

## Orchestrator

The orchestrator is AgentHorsy's task decomposition and dispatch system. When you give it a complex request, it breaks the work into subtasks and dispatches them to the most appropriate agents.

### How it works

1. **Task decomposition** — The orchestrator analyzes your request and breaks it into independent subtasks based on heuristics (file boundaries, domain areas, description keywords).
2. **Agent matching** — Each subtask is matched to the best agent using keyword overlap, description matching, and todo context analysis.
3. **Task dispatch** — Subtasks are sent to agents via the Task tool. The orchestrator manages agent reuse (same agent handles related subtasks) and tracks completion.
4. **Result aggregation** — As agents complete, the orchestrator aggregates their results into a summary, forwarding context between sequential tasks.
5. **Failure recovery** — If a subtask fails, the orchestrator records the failure and can retry or skip based on the error type.

### Configuration

The orchestrator runs automatically when the main agent receives a complex request. It uses the `plan` and `build` agents for task execution, and can dispatch to custom agents via the Task tool.

### Context forwarding

The orchestrator maintains task history and forwards completion summaries between subtasks. This allows later tasks to build on the results of earlier ones without re-exploring the codebase.

### File structure

- `plans/<task-name>.md` — Plan files created by dynamic_persona agents
- `architecture/` — Architecture documentation updated after significant changes
- `database_metadata/` — Database schema documentation updated after migrations
...
Configure agents in your `agenthorsy.json` config file:
...
- Global: `~/.config/agenthorsy/agents/`
- Per-project: `.agenthorsy/agents/`

```markdown title="~/.config/agenthorsy/agents/review.md"
---
...
If no temperature is specified, AgentHorsy uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global AgentHorsy config and the project specific config.
...
The model ID in your AgentHorsy config uses the format `provider/model-id`. For example, if you're using [AgentHorsy Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="agenthorsy.json" {3-6,9-12}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "tools": {
    "write": true,
...
```json title="agenthorsy.json" {3-5,8-10}
{
  "$schema": "https://agenthorsy.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="agenthorsy.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `agenthorsy models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/agenthorsy).
:::
...
```markdown title="~/.config/agenthorsy/agents/docs-writer.md"
---
...
```markdown title="~/.config/agenthorsy/agents/security-auditor.md"
---
