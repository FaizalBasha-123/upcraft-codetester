---
title: Support ACP
description: Utilisez AgentHorsy dans n’importe quel éditeur compatible ACP.
---

AgentHorsy prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser AgentHorsy via ACP, configurez votre éditeur pour exécuter la commande `opencode acp`.

La commande démarre AgentHorsy en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "AgentHorsy": {
      "command": "/absolute/path/bin/agenthorsy",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « AgentHorsy » dans le sélecteur d'agent AI Chat.
...
Pour utiliser AgentHorsy comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser AgentHorsy comme agent ACP pour le chat.
...
AgentHorsy fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration AgentHorsy
