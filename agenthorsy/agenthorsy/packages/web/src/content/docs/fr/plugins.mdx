---
title: Plugins
description: Écrivez vos propres plugins pour étendre AgentHorsy.
---

Les plugins vous permettent d'étendre AgentHorsy en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de AgentHorsy.
...
- `~/.config/agenthorsy/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/agenthorsy/node_modules/`.
...
1. Configuration globale (`~/.config/agenthorsy/agenthorsy.json`)
2. Configuration du projet (`agenthorsy.json`)
3. Répertoire global des plugins (`~/.config/agenthorsy/plugins/`)
...
AgentHorsy exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
:::note
Si vous utilisez l'application de bureau AgentHorsy, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://agenthorsy.ai/docs/sdk) pour plus de détails.
