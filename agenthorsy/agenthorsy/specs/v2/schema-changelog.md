...
- Canonical tagged tool file sources in `@agenthorsy-ai/llm`.
...
- Spill oversized model-facing tool text into globally unique files under AgentHorsy's shared tool-output directory.
...
- Canonicalize ambient instruction traversal boundaries, honor `AGENTHORSY_DISABLE_PROJECT_CONFIG`, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
